...
pub(crate) struct ClientHandler {
    ...
    cols: u16,
    rows: u16,
    /// The pushed per-player init.txt content, decoded from the client's env
    /// request (`Some("")` = clear the stored file). `None` when the client
    /// never sent one (an older late-ssh): leave any existing file alone.
    rc: Option<String>,
    /// The running crawl child, once the shell is requested.
    host: Option<PtyHost>,
}
...
/// The TERM to hand the crawl child: the client's own if the host can resolve
/// its terminfo, otherwise a universal fallback. crawl (ncursesw) aborts on an
/// unrecognized TERM, but every modern terminal renders `xterm-256color`, so
/// this keeps clients on exotic terminals (kitty/ghostty/wezterm, which ship
/// their own terminfo) playable.
...
impl Handler for ClientHandler {
    ...
    async fn channel_eof(
        ...
        // Client hung up; dropping the host tears the bridge down (SIGHUP-save).
        self.host = None;
        Ok(())
    }
