...
/// Read chunk size while catching up. Backfill of a large logfile streams in
/// these increments; `Handle::data` awaits SSH window credit, so a slow
/// client naturally backpressures the reads.
const READ_CHUNK: usize = 64 * 1024;

/// Parse the client's cursor env value (`logfile:123,milestones:456`).
...
/// that tails the xlog files into the SSH channel; dropping the host (client
...
async fn pump(tail: &mut FileTail, handle: &Handle, channel: ChannelId) -> Result<(), ()> {
    // Missing file is normal: crawl creates logfile/milestones on first use.
