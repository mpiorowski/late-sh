...
- Last updated: 2026-08-06 (Pinstar removed entirely and page 5 redesigned as **Profiles**: `Screen::Pinstar` renamed to `Screen::Profiles`, the `late-ssh/src/app/pinstar/` slice, `PinstarServerRegistry` plumbing, the `DELETE_PINSTAR_GRAPH` cap, and the three `pinstar_*` tables (drop migration 135) are gone. The page is now one merged newest-first feed of showcases + work profiles with an all/projects/people filter cycle, a persistent detail panel with per-project author cards (`ShowcaseFeedItem` gained `author_profile`), `o` opens the author's profile modal, and entering the page marks both feed-read cursors)
- Status: Active
...
| **Profiles** | 5 | Active | One merged, newest-first feed of shared projects (showcases) and open-to-work cards (work profiles), with a filter cycle (all / projects / people) on `[` / `]` or idle `h` / `l` and a persistent detail panel at >=86 cols. The detail panel previews the public web profile sections (work fields, Settings Bio, late.fetch, projects); a selected project also shows an author card. `i` shares a project, `w` creates/edits your work card, `o` opens the author's profile modal, `s` searches both kinds, `/` filters to your own entries, Enter copies the row's link. Entering the page marks both feed cursors read. Lives in `late-ssh/src/app/directory/` (module keeps its old name). |
