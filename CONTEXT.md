...
- Last updated: 2026-08-31 (first contact, the whole haunting ladder, admin-scoped, in the new `late-ssh/src/app/deadchannel/` game domain (subdomain `deadchannel/haunt/` owns onboarding; `glyphs.rs` holds the game's shared glyph alphabet). Stage 1: the sidebar clock glitch, a scheduled ~200ms swap of one or two time characters for glyph-alphabet characters, per-session dice, rare (roughly one per hours-long session, at most twice a UTC day), only while the clock is on screen, render-only. Stage 2: the own-name flicker, rolled on the landing echo of your own send (~1 in 24, one hit a day, three ever), corrupting that message's author label for ~300ms via the chat rows cache key; every hit bumps the persisted `first_contact_name_hits` counter, which is what arms stage 3 for the next fresh connect. Stage 3: the splash whisper (the held door): the splash neither skips nor expires while held, input is acknowledged (static surge, hint dissolve) but never obeyed, the voiced line types itself, a ~10s hard cap always releases; delivery stamps `first_contact_whisper_at`. Stage 4: the invitation, `INVITE_DELAY_DAYS` after the delivered whisper: the game's first voice (`afterglow`, a bartender-shaped ghost user that never auto-joins public rooms) sends one persistent DM ending in `/join #deadchannel`; a conditional settings claim (`first_contact_invited_at`) keeps racing devices to one DM. All of it behind the process-global in-memory kill switch (`State.haunt_enabled`) and armed only for admins; `/haunt` (parsed only for admins, a non-admin's posts as plain text) covers status/on/off plus per-stage test hooks (glitch/name/replay/invite/reset). The root keeps one field and one routing call per file; orchestration lives in `deadchannel/haunt/svc.rs`. `late-ssh/src/app/deadchannel/CONTEXT.md`.)
- Status: Active
