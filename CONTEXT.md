...
| `late-ssh/src/app/lobby/CONTEXT.md` | The Lobby: the `Ctrl+G` modal and `LobbyState`/`LobbyEntry` over the three game domains. | Lobby domain overview and the modal contracts; routes onward to `lobby/daily/CONTEXT.md`, `lobby/realm/CONTEXT.md`, and `lobby/house/CONTEXT.md`. |
| `late-ssh/src/app/lobby/realm/CONTEXT.md` | Realm territory-conquest games: rulesets, the instant-resolution engine, distance-decayed odds, the Earth map assets + `tools/realm-mapgen`, `Screen::Realm`, ranked chip payouts, or the `realm_games`/`realm_actions`/`realm_days` tables. | Realm domain context: rules-as-data (`RealmRuleset`), the pure engine (`resolver.rs`, seeded SplitMix64, first-come-first-served races, refusal semantics), split hot/cold persistence with revision CAS, the map asset format + generator, the map/overview/log screen, modal integration, payout tiers, invariants, and deferred hooks (per-game chat, terrain). |
...
| RealmGame | `realm_games` + `realm_days` | Realm conquest games (migrations 192/193, reshaped by 195, named by 196): one row per game (`status`, `name` — the creator's label, blank means "<creator>'s realm", `ruleset_id`, `reset_hour_utc` — the hour this game's daily action points refill, `last_resolved_day` = last realm day swept, state JSONB with revision CAS: ruleset snapshot, players, ownership, budgets, live day log), plus a per-day log archive. Actions resolve the instant they are taken and are settled by that revision CAS, so migration 195 dropped the old hidden-queue table `realm_actions`. Ranked finish payouts seeded by migration 194 and rescaled by 197 (pot = 3000 chips × players at start, split by place, withheld from games that ended before they were played), paid with `ChipMove::RealmConquest`. See `late-ssh/src/app/lobby/realm/CONTEXT.md`. |
...
| _Realm_ | — | Active | Full-screen realm territory-conquest game (`Screen::Realm`: continuously zoomable Earth map with territory outlines, target table, standings, day log), not a top-level tab. Entered only from the Lobby modal; `q`/`Esc` returns to the modal. Lives in `late-ssh/src/app/lobby/realm/`. |
