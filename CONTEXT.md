...
- Last updated: 2026-08-10 (crawl-file publishing ships, the last phase of `devdocs/PLAN-ROGUELIKE-BOARDS.md`: the `late-dcss` pod runs a second, read-only HTTP listener (`late-dcss/src/publish.rs`) serving crawl's `logfile`, `milestones`, and morgue dumps under `late.sh/crawl/...` for the public DCSS tooling (dcss-stats, Sequell), fronted by a path-prefix ingress rule that reuses the apex host's existing certificate. Same bytes our own ingestion tails, so the two validate each other. The roguelike-boards plan is now complete in code; registering late.sh with that ecosystem is a conversation, not a build)
- Status: Active
