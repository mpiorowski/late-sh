...
- Last updated: 2026-08-25 (the Shop sells exactly one title, Your Own Title: migration 151 retires the 72 curated title rows from 149 with `active = false` and renames the custom pair; the Shop tabs now run Chat, Badges, Flags, Companions, Aquarium, Ultimates, and the Chat list is split by section rows into Name effects / Title / Consumables (`late-ssh/src/app/hub/CONTEXT.md`). Previous: the Shop sells a title you write yourself: migration 150 adds `title_custom_day` (2,000) and `title_custom_month` (60,000), whose text the buyer types into a Shop prompt. `CustomTitle::parse` is the one validator (20 characters, no controls, no `@`) and `late-ssh/src/app/ai/screen.rs` screens the text with an ungrounded schema-enforced call that fails closed, both before the purchase transaction opens: a refused title costs nothing, and with no AI configured the SKUs render as unavailable rather than shipping unscreened. Previous: the Shop's cosmetics became rentals. Badges and flags now sell in a 24h and a 30-day tier (migration 148, 100/3,000 basic and 250/7,500 premium, flags at the basic price) and the permanent SKUs are retired with `active = false`; the chat label query prefers a live rental and falls back to whatever a legacy owner still has equipped, so nobody loses a badge they bought. A new `title_rental` kind sells 36 curated noir titles (migration 149, 200/24h and 6,000/30d) that print as `, <title>` after the name in chat and on the clubhouse floor, in their own slot so a title and a username color never clear each other. `late-core/src/models/rental.rs` holds the shape all four rental kinds share: the two windows, the copy that quotes them, and the payload readers. Previous: arrow keys in the doors, and a `/summary` floor. Every vt100-backed door now retypes cursor keys to the mode the game asked for (shared `late-ssh/src/app/door/keys.rs` gated on `Screen::application_cursor`; a no-op for games that never request it; invariant write-up in `late-ssh/src/app/door/dcss/CONTEXT.md` §4). NetHack needed a windowport change too: the default tty port reads raw `getchar()` and decodes no arrow escapes in any mode, so the `door-nethack` image (tag `5.0.0-r3`) now also compiles the curses windowport, a per-player opt-in via `OPTIONS=windowtype:curses` in the rc paste box; tty stays everyone's default (`late-ssh/src/app/door/nethack/CONTEXT.md` §6/§7). `/summary` now always covers at least the last 24h, widened by an older read marker and still capped at 48h / 200k chars: the read cursor advances just from sitting in a visible room, so an overnight-open terminal used to get "nothing new to summarize". A window can also be typed (`/summary 6h`, `/summary 90m`), which is taken at face value up to the same 48h cap. Policy lives in `summary::window_start` over a closed `SummaryWindow` (`late-ssh/src/app/chat/CONTEXT.md` §14).)
- Status: Active
