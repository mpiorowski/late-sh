...
- `--ssh-mode native` is a rollout path, not yet the default; `--ssh-mode subprocess` remains the compatibility fallback. Native mode now requires a server that supports the `late-cli-token-v1` SSH exec handshake.
