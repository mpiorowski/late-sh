...
│   │       ├── goldfish/       # Persistent goldfish bowl state, service, and UI
...
| CatCompanion | `cat_companions` | `user_id` UNIQUE, nullable `last_fed`/`last_watered`/`last_played` timestamps; mood derived from hours since most recent care, never dies |
| GoldfishBowl | `goldfish_bowls` | `user_id` UNIQUE, nullable `last_fed`/`last_decorated`/`last_lit`/`last_water_changed` timestamps, `friend_count` INTEGER clamped to MAX_FRIENDS (5); mood derived from hours since most recent care, never dies |
