...
- Last updated: 2026-08-24 (arrow keys now work in the Brogue and DCSS doors. Both games ask the terminal for application cursor keys (`keypad(TRUE)` → `smkx`); that request died in our `vt100::Parser`, so the client kept sending `ESC [ A` while ncurses only decodes `ESC O A`, and an Up arrow reached brogue as ESC + `[` + `A` (cancel + "turn on autopilot?"). The doors now retype the four arrows plus Home/End to match the live mode off the parser: shared `late-ssh/src/app/door/keys.rs`, wired through each door's `keys_for_game`. NetHack is untouched: its tty windowport decodes no arrow escapes in any mode, so that would be a windowport change, not this fix. Details in `late-ssh/src/app/door/dcss/CONTEXT.md` §4.)
