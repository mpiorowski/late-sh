...
- Last updated: 2026-08-23 (chat catch-up trio: `j`/`k` scroll by rows inside a selected message taller than the pane before moving selection; the `/history` modal gained the `new messages` divider and opens on the first unread; new `/summary` command asks the AI for a public-room catch-up since the last read, capped at 48h / 200k transcript chars with cooldown + daily cap (`late-ssh/src/app/ai/summary.rs`). Details in `late-ssh/src/app/chat/CONTEXT.md` §9 and §14)
- Status: Active
