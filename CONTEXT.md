...
- **Pinned chat messages are generic but membership-scoped:** `chat_messages.pinned` is a message-level flag, not tied to `#announcements`. Only admins can toggle it (`Ctrl+P` on the selected message). Dashboard rendering stacks all pinned messages returned for the current user above the normal dashboard chat, and both the DB query plus live `MessagePinUpdated` event targeting are membership-scoped so pinned DMs/private rooms do not leak to non-members.
