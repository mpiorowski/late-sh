...
- Last updated: 2026-08-07 (Leaderboards page grows a Games section that leads the board rail: two Lateania snapshot boards (Adventurers by level with the class as a row note, Frontier by deepest zone) read from the `mud_characters` blobs; the leaderboard pass is eleven queries now. Roguelike-door boards/badges are planned in `devdocs/PLAN-ROGUELIKE-BOARDS.md`)
- Status: Active
