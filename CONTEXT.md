...
In progress:
- **Rooms/Blackjack done:** Rooms is now the primary multiplayer table-game entry point (`Screen::Rooms`, key `4`). `RoomsService` creates persistent Blackjack tables in `game_rooms` and associated `chat_rooms(kind='game')` rows in one SQL CTE, then publishes snapshot refreshes plus `RoomsEvent` success/error. The Rooms page supports row selection, admin-only room creation, admin/mod room entry, and returning to the list with `Esc`. Active room mode captures input after modals/global `Ctrl-O`, so game/room keys do not leak to app-wide hotkeys (`w`, `P`, `?`, `1-5`, `Tab`, etc.).
- **Blackjack table runtime done:** `BlackjackTableManager` lazily maps each entered Blackjack `GameRoom.id` to its own runtime `BlackjackService`. Blackjack room creation uses a dedicated modal for table name, pace (`quick`, `standard`, `chill`), and stake (`10`, `50`, `100`, `1000` chips); those settings are persisted in `game_rooms.settings` and applied when the room runtime is first created. Entering a room starts as a viewer; `s`/Enter takes the first open seat, up to four seats. Seated players build a stake from a chip rack instead of typing numbers: `[ ]` or `A/D` selects a chip, Space tosses it onto the stake, Backspace pulls one chip back, `c` clears the stake, and Enter locks it in. Each confirmed stake restarts the table's pace-specific betting window so other seated players can still bet concurrently until auto-deal. After the deal, all betting seats can hit/stand their own hands in parallel; the dealer resolves once every betting hand has stood, busted, or naturally settled. `l` leaves the seat when safe, and `Esc` leaves the room/view. Blackjack now supports multi-seat bets, per-seat hands, parallel player actions, dealer resolution, and per-user chip settlement.
- **Rooms/Blackjack not done:** Server restart still drops in-memory Blackjack table state; entering an existing room creates a fresh runtime service. Future durable games should restore from game-specific persisted state inside their manager. The active room bottom half is still a placeholder showing the associated `chat_room_id`; real room-scoped chat rendering is not wired yet. AFK/disconnect handling is still missing.
