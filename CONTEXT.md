...
- Last updated: 2026-07-31 (Leaderboards became top-level page `6` fed by roster-generated boards (`DailyPuzzle`/`ScoreGame` in `late-core/src/models/leaderboard.rs`); the Hub modal reduced to the Shop (+admin tab), opened by `/shop`, no chord; daily quests render at the top of The Arcade; the Lobby moved to `Ctrl+G` and `Ctrl+Q` is unbound; the Le Word streak board and hub leaderboard grid were deleted. CodeKeep: The Pale added as a dedicated network door: lockfile-pinned `codekeep@1.0.9`, `late-codekeep` SSH/PTY host, immutable-account HOME saves, and `Screen::Codekeep` + `HubGame::Codekeep`)
- Status: Active
