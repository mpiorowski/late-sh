...
- Last updated: 2026-08-08 (the NetHack log pipe ships, Phase 2 of `devdocs/PLAN-ROGUELIKE-BOARDS.md`: `late-nethack` gains the reserved `late_stats` stats session streaming `xlogfile` + `livelog` (livelog newly enabled in the build) to the shared ingestion service in `app/door/ingest/`, which lands NetHack `door_runs`/`door_milestones`, feeds the NetHack board triple on the Leaderboards page, grants the lifetime NHA/NHY badge pair from the pipe, and posts death/win feed events. The old vt100 screen scrape (`door/nethack/{milestone,status,award}.rs` + `scan_screen`) is deleted; the door no longer has a persistence exception. Brogue (Phase 3) and dcss-stats HTTP publishing (Phase 4) remain)
- Status: Active
