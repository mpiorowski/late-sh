...
| `late-ssh/src/app/clubhouse/nightcap/CONTEXT.md` | Nightcap, the small bar out back of the Clubhouse (`n` to enter, Esc to leave): six stools, a hidden `kind='nightcap'` room only the seated may speak in, a fixed drink menu on the tavern's chip and drunk rails, `Screen::Nightcap`. | The hidden-room contract (`is_chat_list_room`), the process-global `SharedSeats` contract (single-replica, same as the Clubhouse), how orders settle off-thread, and the module map. |
