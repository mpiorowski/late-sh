...
- Last updated: 2026-09-11 (The Home Lounge companions are back: `/aquarium` (alias `/aq`) toggles the Shop-unlocked tank's tray above the lounge chat and `/pet` the pet strip above its composer, both persisted per account (`show_aquarium_tray`, `show_pet_strip`, with the Tweaks row); the Zen tiles are unchanged, and a click on the Zen pet pets it without focusing its tile. Earlier: the global chords have typed composer fallbacks for terminals that swallow them: `/lobby` (Ctrl+G), `/zen` (Ctrl+F), `/redraw` (Ctrl+L), `/guide` (?), alongside the existing `/settings` (Ctrl+O); each runs the same function as its key. See §Keyboard shortcuts. Ideas and roadmap live in `PLAN.md` Backlog; this file describes the present only.)
- Status: Active
...
| `late-ssh/src/app/hub/CONTEXT.md` | The `/shop` Shop modal, the Arcade quest strip's service, marketplace, pet/aquarium unlocks, or chip economy presentation. | Shop modal ownership, reward/economy rules, daily/weekly quest service, marketplace and entitlement projection, the Zen and Home Lounge tank and pet surfaces, and known gaps. |
