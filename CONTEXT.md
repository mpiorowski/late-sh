...
3. **SSH transport modes** — `late-cli` now has a runtime transport switch, but native is the default. `--ssh-mode old` preserves the original OpenSSH-through-pty path, `--ssh-mode openssh` uses a system OpenSSH ControlMaster flow for OpenSSH-managed auth such as YubiKey/FIDO security keys, and `--ssh-mode native` uses an embedded `russh` client. The old path still intercepts the one-line `LATE_SESSION_TOKEN=<base64url-uuid-v7>` banner from the PTY stream. The openssh and native paths both use the dedicated `late-cli-token-v1` SSH `exec` request, expect JSON `{ "session_token": "..." }`, and then open the real interactive shell with no banner parsing and no fallback. In old/native modes stdin stays blocked until the token phase completes so pre-handshake keystrokes do not leak into the app; openssh mode lets OpenSSH own the terminal during auth so PIN/passphrase/touch prompts behave normally.
4. **Identity + host trust + pairing** — Client identity remains SSH-key based, but the interactive fallback now generates `~/.ssh/id_late_sh_ed25519` natively instead of shelling out to `ssh-keygen`. Operators can override the key path with `--key` or `LATE_KEY_FILE`; if the file does not exist and the terminal is interactive, the CLI offers to generate the key at that exact path. `LATE_IDENTITY_FILE` is still accepted as a legacy env fallback. OpenSSH-managed mode is the exception: when no explicit key is provided it lets OpenSSH use normal `~/.ssh/config`, agent, and default identity discovery. Native mode verifies server keys against `~/.ssh/known_hosts` and learns first-seen keys with accept-new semantics. The CLI then uses `/api/ws/pair` to forward analyzer frames, accept paired control commands, and report `client_state { client_kind, ssh_mode, platform, muted, volume_percent }`.
...
- Native SSH is now the default launcher path; `--ssh-mode old` remains the legacy compatibility fallback, and `--ssh-mode openssh` is the OpenSSH-managed path for hardware-backed keys. Native and openssh modes require a server that supports the `late-cli-token-v1` SSH exec handshake.
...
- **CLI binary must forward local terminal resizes:** The `late` binary runs SSH inside a local PTY in legacy old mode and over a `russh` channel in native mode; both paths must propagate local terminal resizes so side-by-side panes and split windows reflow correctly after startup. OpenSSH-managed mode delegates terminal resize handling to the system `ssh` client.
- **CLI must suppress pre-token input where it owns input forwarding:** The local `late` binary must not forward stdin into the interactive SSH PTY/channel until the pairing token exchange has completed. In subprocess mode that still means waiting for the server's `LATE_SESSION_TOKEN=` banner; in native mode it means waiting for the dedicated `late-cli-token-v1` SSH exec handshake response. Any keys typed during handshake/welcome race windows are intentionally discarded, and pending terminal input is flushed immediately before forwarding starts. OpenSSH-managed mode is the exception because the system `ssh` client owns terminal input during auth and only opens the interactive shell after token acquisition.
