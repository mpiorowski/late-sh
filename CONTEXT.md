...
- Last updated: 2026-08-07 (the DCSS log pipe ships, Phase 1 of `devdocs/PLAN-ROGUELIKE-BOARDS.md`: a reserved `late_stats` SSH session on `late-dcss` streams crawl's xlog files to a late-ssh ingestion task (`app/door/ingest/`), landing `door_runs`/`door_milestones` facts that feed the DCSS board triple in the Leaderboards Games section (Wins/Deepest Dive/Top Score; the pass is thirteen queries now), the lifetime `DCO`/`DCW` badge pair with 10k/20k chip payouts, and #lounge death/orb/win feed events. NetHack and Brogue phases are next in the plan. The leaderboard domain also got its own slice: `LeaderboardService` moved to `app/leaderboard/svc.rs` and its context to `app/leaderboard/CONTEXT.md`)
- Status: Active
