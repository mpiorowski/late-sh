...
- **The CLI analyzes, the server only draws:** `viz` frames carry the paired CLI's FFT of its audible Icecast/radio output. They update `AudioState`'s spectrum without dirtying the app, expire after 750ms of silence, and never exist for YouTube (the iframe's audio is unreadable), which keeps the wall-tick ambient band; see `late-ssh/src/app/audio/CONTEXT.md` §10.
