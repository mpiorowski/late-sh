...
- **Pet Companion is a Hub Shop unlock:** `pet_companions` stores care timestamps, cat/dog species, and the completed-care streak used for `Happy`. Visibility/access is gated by Hub Shop entitlements from `user_purchases`. The global `c` opens pet care only after SKU `cat_companion` is owned; while locked, `c` is inert and the sidebar points users to `Ctrl+G` for Hub Shop. The sidebar renders the pet slot as locked until `ShopEntitlements::has_cat_companion()` is true.
