...
- Last updated: 2026-09-06 (Profile modal redesign: one scrolling layout on every terminal, the bonsai as the late.fetch logo, every badge listed, the public chips ledger at the bottom, `/chips [@user]` jumps to it; see `late-ssh/src/app/profile_modal/CONTEXT.md`. Chip ledger provenance and the simpler Top Chips rule: every `chip_ledger` row now carries a `source_ref` (`UserChips::apply` requires it), the house tables write per-game reasons with a round/hand id, gifts name the counterparty, the starting stipend is a ledger row (`ChipMove::InitialBalance`), and Top Chips counts everything in `chip_ledger` on both sides except blackjack, poker (bets, payouts, floor restores) and gifts. Admin chips are minted by the admin-only `/grant` chat command with no ledger row by decision; `scripts/add_admin_chips.sh` is gone.)
- Status: Active
...
| `late-ssh/src/app/pot/CONTEXT.md` | The weekly pot: `/pot` and `/pot buy N`, `PotService`, the `pots` / `pot_tickets` tables, the status HUD badge, the draw sweeper, or the pot's #lounge lines. | Pot domain context: the fixed numbers (100-chip tickets, 10 per player per UTC day, 80% payout, Monday 21:00 UTC), the two `ChipMove` variants (both count on Top Chips since 2026-09-06), the three concurrency guards (partial unique index, advisory lock, status transition), the `pot_changed` distribution model and the private holdings map, the status HUD badge (there is no sidebar panel any more), the draw's feed line and headline, telemetry, and the gotchas. |
...
# migrations and app flows. Chips for a local account come from the admin-only
# /grant chat command.
