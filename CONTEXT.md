...
- Last updated: 2026-07-13 (Lobby consolidation phases 3+4: the Rooms domain is GONE — `app/rooms/` deleted wholesale (screen, directory, `RoomsService`, manager traits, create modals, dead Chess/Tic-Tac-Toe/ssHattrick runtimes), `Screen::Rooms` removed and the tab bar renumbered (Artboard `4`, Directory `5`, World Cup `6`), the home multiplayer box + `b1`-`b4` shortcuts removed, migration 111 drops `game_rooms` + rooms-era game chat rooms/voice channels, quests' `room_rounds_played`/`room_wins` arms and `ActivityKind::GamePlayed` deleted, @dealer re-pointed at the house blackjack singleton. Phase 4: `app/daily/` + `app/house/` now live under `late-ssh/src/app/lobby/` (`lobby/daily/`, `lobby/house/`), the `Ctrl+Q` modal is lobby-owned (`lobby/modal_input.rs`/`modal_ui.rs`, `LobbyEntry`), new `App::lobby: LobbyState` owns modal cursor/claim-confirm/glow, and the backtick cycle moved to `lobby/workspace.rs`. Same day, earlier: phase 2 house tables + Runbook §10.5 incident log; 2026-07 OOM follow-up shipped clipboard request-gating on the pair WS, SSRF-guarded RSS fetches, per-feed RSS fairness, best-effort `BrowserPaired` routing. Previous update 2026-07-12: report-only feedback rooms: `#bugs`/`#suggestions` accept only `/bug <text>` / `/suggest <text>` report cards from regular users — the commands work from any room and post a `---BUG---`/`---SUGGESTION---` marker-bodied message rendered as a ruled card (same body-marker trick as `---NEWS---`); free-text sends there are rejected for non-staff while admins/moderators keep plain-text replies and everyone keeps reactions for "+1"; no schema changes, see `late-ssh/src/app/chat/CONTEXT.md` §8. Same day: activity feed became #lounge system messages and the sidebar Activity panel was retired; see the `Activity` service row and `late-ssh/src/app/chat/CONTEXT.md`)
- Status: Active
