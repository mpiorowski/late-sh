...
- Last updated: 2026-07-30 (Leaderboards became top-level page `6` fed by roster-generated boards (`DailyPuzzle`/`ScoreGame` in `late-core/src/models/leaderboard.rs`); the Hub modal reduced to the Shop (+admin tab), opened by `/shop`, no chord; daily quests render at the top of The Arcade; the Lobby moved to `Ctrl+G` and `Ctrl+Q` is unbound; the Le Word streak board and hub leaderboard grid were deleted)
- Status: Active
...
| `/shop` | Chat composer | Open the Shop modal (`Shop` tab, plus `Admin` for admins). The Shop has no global chord; the locked pet/aquarium nudges open the same modal programmatically |
