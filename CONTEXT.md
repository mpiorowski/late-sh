...
- Last updated: 2026-08-26 (the crown, the Shop's second sink outside the Shop and the first one nobody prices: one slot, one holder, one ♔ glued to their name in every chat author header and on the Clubhouse floor. `/crown` prints who wears it and what taking it costs; `/crown take` buys it for `max(5,000, ceil(paid x 1.5))` (`late-core/src/models/crown.rs`, migration 156), so the price ratchets with whoever last paid and we never tune it. Every chip is destroyed: `ChipMove::CrownTaken` is a debit with no matching credit reason anywhere. A fresh reign holds for 30 minutes, you cannot take a crown you already wear, and the slot empties at the UTC month rollover with no sweeper (a reign is current only while its month is), leaving the month's last holder the permanent `CRWN` profile award. The glyph rides the once-a-second `name_flair` resolution off a process-shared holder kept in step by the `crown_changed` Postgres notify, so no render ever queries for it. Every takeover posts to #lounge naming both players. `late-ssh/src/app/chat/CONTEXT.md` §9c.)
- Status: Active
...
- `CrownService` (in `app/crown/svc.rs`) owns the crown: one row in `crown_reigns` (`late-core/src/models/crown.rs`, migration 156) held by one user, taken off them for `max(5,000, ceil(paid x 1.5))`. `ChipMove::CrownTaken` is a floor-guarded debit with no matching credit reason anywhere, so the whole price leaves the money supply; the ladder therefore ratchets with the buyers and is never tuned here. The take transaction takes a `pg_advisory_xact_lock` before the `SELECT ... FOR UPDATE`, because a take from a vacant crown has no row to lock and two racing first takes would otherwise collide on the `crown_reigns_single_open` partial unique index instead of settling as one debit and one hold refusal. Distribution is the `StreamService` shape: a process-shared `watch<Option<CrownHolder>>` seeded by a `crown_changed` LISTEN/NOTIFY task (`start_listener_task`, wired in `main.rs`, reconnecting and re-seeding after 5s) plus a `broadcast<CrownEvent>` for the two command answers. A reign carries the first of the UTC month it was taken in and counts only while that month does, so the slot empties at the rollover with no sweeper and the month's last holder gets a `crown` profile award (`CRWN`, rankless) from the existing monthly snapshot. The glyph is folded into `App.name_flair` on the same once-a-second edge as the flair directory, so no render queries for it. Commands `/crown` and `/crown take`; every takeover posts an `ActivityKind::CrownTaken` line naming both players. Full contract: `late-ssh/src/app/chat/CONTEXT.md` §9c.
