...
- Last updated: 2026-07-02 (World Cup HUD upcoming kick-offs now render in the viewer's account timezone when set, falling back to UTC; World Cup HUD as top-level screen `7`, a demand-gated FotMob live poller; dopewars door: prod infra + dedicated CI/CD — `infra/service-ssh.tf`, `.github/workflows/dopewars.yml`; Clubhouse tavern as admin-gated top-level screen `0`: zoomed free-roam ASCII Late Lounge generated by `scripts/gen_clubhouse_map.py`, live-user seating, page-signpost landmarks, embedded #lounge chat, and the new @bartender ghost bot)
- Status: Active
...
- **Clubhouse tavern (admin preview):** Top-level screen `0`, gated by `is_admin` (the `b'0'` arm in `handle_global_key`; losing admin bounces you to Home). Full-bleed walkable ASCII tavern (`late-ssh/src/app/clubhouse/`: `map.rs` floor plan + collision/seats/zones, `state.rs` avatar/roster/rotation, `input.rs`, `ui.rs`) over an embedded #lounge chat (bottom ~40%, same `EmbeddedRoomChatView` machinery as Rooms with its own `clubhouse_chat_rows_cache`). The floor plan is a 184x50 grid — larger than most viewports, so the camera pans with the player — drawn Dwarf-Fortress-style with single-width glyphs only (no `☺`-class chars, they render double-width in some terminals): people are 3-row stick figures (`o` head, `/|\` arms, `Λ` legs; the player is `@`), stools are `(_)` on a `╨` leg (occupants perch as an `o` above; `Seat.kind` distinguishes stools from armchairs), the dog by the hearth is the 3-row `( o.o )/` sprite with blink/tail-wag/z animations. Movement is free-roam: only the outer walls and the bartender's alley block; everything else (tables, fire, counter top) is climbable. **The map literal is generated — do not hand-edit `MAP` in `map.rs`; tweak and re-run `scripts/gen_clubhouse_map.py --write`** (it validates row widths, seat anchors, and flood-fills the plan: sealed alley, reachable seats/spots; its zone constants and test probe coordinates are hand-synced from the numbers it prints). Unit tests in `map.rs` mirror those checks. Live humans from `active_users` are seated in arrival order (66 seats → 6 standing spots → `+N at the door`; over capacity the crowd rotates one place every ~25s). The tavern draws no widget border — the room's walls are the frame — and its headcount + keybinds live in the app frame title (`app_frame_title` in `render.rs`). Interactive props: bartender (`t` or Enter prefills `@bartender `), jukebox (music-key cheat sheet; Enter opens the Music Booth), and one red-framed signpost landmark per app page with its page number baked into the art (`ARCADE·2`, `DOORS·3`, `TABLES·4` on the poker felt, `ARTBOARD·5` on the easel) where Enter jumps to that page; the fireplace and the dog have flavor popovers. @bartender ghost bot (warm tavern keeper, carries `bot_app_context()`) answers mentions. Before opening to everyone: drop the admin gate, decide seat-capacity behavior at real user counts, and consider a splash tip + help-modal entry.
