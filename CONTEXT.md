...
| `late-ssh/src/app/pot/CONTEXT.md` | The daily pot: `/pot` and `/pot buy N`, `PotService`, the `pots` / `pot_tickets` tables, the sidebar Pot panel, the draw sweeper, or the pot's #lounge lines. | Pot domain context: the fixed numbers (100-chip tickets, 50 per player, 80% payout at 21:00 UTC), the two `ChipMove` variants and why neither counts as earnings, the three concurrency guards (partial unique index, advisory lock, status transition), the `pot_changed` distribution model and the private holdings map, the two-row panel, the drawn/threshold feed lines, telemetry, and the gotchas. |
...
| Pot | `pots` | The daily raffle (migration 160). `status` IN (`open`, `drawn`, `rolled`) with a partial unique index on `(status = 'open')` so at most one is open ever; `draws_at` is the next 21:00 UTC, `ticket_price` 100. `winner_user_id`/`ticket_count`/`payout_chips`/`drawn_at` are all NULL while open and stamped once at the draw, enforced by one CHECK per status (a drawn pot is told from a rolled one by `ticket_count > 0`, not by the winner, whose account can be deleted out from under a settled row). `announced_threshold` is the high-water mark that makes the 50,000 / 100,000 #lounge lines once-per-pot across replicas and restarts. |
| PotTicket | `pot_tickets` | One row per buy, never updated: a player's holding is the sum of their rows, and `chip_ledger` has one `pot_ticket` debit per row. A live pot's size is `SUM(count) * ticket_price`; there is no stored running total and no house wallet. The 50-per-player cap is checked inside the insert's `WHERE` and made exact by the buyer holding the pot row `FOR UPDATE`. |
