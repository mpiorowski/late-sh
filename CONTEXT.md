...
- Last updated: 2026-07-29 (Chip economy hardening: `ChipMove` closed enum + `UserChips::apply` choke point in `late-core/src/models/chips.rs`; `chip_user_changed` is now guaranteed by `user_chips` triggers in migration 128, fixing /gift never notifying; earnings exclusion lists derive from `ChipMove::excluded_earning_reasons()`)
- Status: Active
