...
| `late-ssh/src/app/zen/CONTEXT.md` | The Zen page (screen `7`): Rice, the per-account tiling layout of the bonsai, tank, pet, the selected room's chat, music, and a clock, and behind `o` the Room, a hand-drawn night room at a fixed size with the same things placed in it. | Tile tree and persistence (`users.settings.zen_layout`), the key map, how the aquarium sim is re-bound to whichever page shows it. |
