...
- Last updated: 2026-08-24 (arrow keys now work in all three roguelike doors. Brogue, DCSS, and now NetHack retype cursor keys to match the mode the game asked for (shared `late-ssh/src/app/door/keys.rs`, wired through each door's `keys_for_game`; invariant write-up in `late-ssh/src/app/door/dcss/CONTEXT.md` §4). For NetHack this needed a windowport change too: the default tty port reads raw `getchar()` and decodes no arrow escapes in any mode, so the `door-nethack` image (tag `5.0.0-r3`) now also compiles the curses windowport, a per-player opt-in via `OPTIONS=windowtype:curses` in the rc paste box. tty stays everyone's default. Details in `late-ssh/src/app/door/nethack/CONTEXT.md` §6/§7.)
