...
- Last updated: 2026-08-08 (the Brogue log pipe ships, Phase 3 of `devdocs/PLAN-ROGUELIKE-BOARDS.md`: `late-brogue` gains the reserved `late_stats` stats session streaming every player's `BrogueRunHistory.txt` to the shared ingestion service in `app/door/ingest/`, which lands Brogue `door_runs`, feeds the Brogue board triple on the Leaderboards page, grants the lifetime BRE/BRM badge pair, and posts death/win feed events. Identity is the per-player directory, not a field on the line, and a build patch restores upstream's broken normal-mode victory logging. All three external roguelike doors now run on the log pipe; only dcss-stats HTTP publishing (Phase 4) remains)
- Status: Active
