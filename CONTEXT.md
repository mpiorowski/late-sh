...
- Last updated: 2026-09-03 (Sliding Puzzle's session-only image-tile view pre-encodes opaque native Kitty/iTerm2/Sixel cells once per source/geometry/protocol. A legal move replaces only the moved tile and gap in place — new Kitty placements are installed before targeted old-image deletion — so input never waits for whole-board recomposition, and Chafa remains initial-loading/unsupported-terminal fallback only. The pre-frame wipe that keeps iTerm2/Sixel pixels from leaking now covers non-modal rasters too, so its vocabulary is `pre_frame_persistent_raster_wipe_bytes` rather than Sixel-specific.)
- Status: Active
