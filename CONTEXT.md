...
| `late-ssh/src/app/pot/CONTEXT.md` | The weekly pot: `/pot` and `/pot buy N`, `PotService`, the `pots` / `pot_tickets` tables, the sidebar Pot panel, the draw sweeper, or the pot's #lounge lines. | Pot domain context: the fixed numbers (100-chip tickets, 10 per player per UTC day, 80% payout, Monday 21:00 UTC), the two `ChipMove` variants and why neither counts as earnings, the three concurrency guards (partial unique index, advisory lock, status transition), the `pot_changed` distribution model and the private holdings map, the two-row panel and the status HUD badge, the draw's feed line and headline, telemetry, and the gotchas. |
