...
| `late-ssh/src/app/jobs/CONTEXT.md` | The job feed: the Jobs shelf on the Profiles page, `/jobs`, the nightly press (HN Who is hiring, We Work Remotely, Jobicy), the tag vocabulary, the FOR YOU matches under a work card, the paper's NEW WORK rows, the `jobs_enabled` flag, or the `job_postings` / `job_press_runs` tables. | The once-a-night press under a daily run claim, the 14-day HN drip, the model read and its schema, the per-replica shelf snapshot, tag matching, telemetry, tests, and gotchas. |
...
| JobPosting | `job_postings` | The job feed's rows (migration 193): `(source, external_id)` UNIQUE, `status` IN (pending, queued, active, expired, dead, dropped), `remote_kind` set on every shelf row, `released_on` set on every active or expired row, `raw` cleared on settle. `JobPressRun` (`job_press_runs`, `run_on` PK) is the nightly press's claim. See `late-ssh/src/app/jobs/CONTEXT.md`. |
