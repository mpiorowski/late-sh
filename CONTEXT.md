...
| PetCompanion | `pet_companions` | `user_id` UNIQUE, nullable `last_fed` (plus unused `last_watered`/`last_played`/`last_treated`/`care_streak_*` columns), `species` (`cat`/`dog`). One free meal per UTC day: `PetCompanion::feed_day` is the atomic first-of-the-day gate, paid `ChipMove::PetFed` (+100) in the same transaction by `PetService`. Mood is binary: fed today is `Happy` (roams its box), otherwise `Sad` (sits on the floor); pets never die. |
