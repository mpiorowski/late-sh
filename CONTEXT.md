...
- Last updated: 2026-07-27 (A Dark Room door added: a native in-process incremental at `late-ssh/src/app/door/darkroom/`, migration 127 `darkroom_saves`, `Screen::Darkroom` + `HubGame::Darkroom`; upstream is MPL-2.0, so `data.rs`/`model.rs`/`sim.rs` carry MPL headers while the rest of the door stays FSL, see NOTICE and LICENSING.md)
- Status: Active
