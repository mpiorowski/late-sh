...
- Last updated: 2026-09-01 (two threads. **Workspace cycle:** Dark Room and Green Dragon joined the backtick workspace cycle as its last stops, and the cycle moved into its own domain, `late-ssh/src/app/workspace/` (`cycle.rs` + the Arcade leg in `arcade.rs`; the non-cycle Arcade residents stayed behind as `arcade/daily.rs`). Door liveness has one definition, `door/hub/state.rs::HubGame::live_screen`, read by both the cycle and the hub sidebar's in-progress pips. The two loaded remakes idle out after `door::game::IDLE_WINDOW` (30 min) away from the door; `App::tick` stamps presence while a remake is the open screen and reaps an expired one with the same settle-and-save an explicit leave runs. Green Dragon refuses the hop mid-fight, Dark Room mid-ascent. See `late-ssh/src/app/workspace/CONTEXT.md`. **First contact (2026-08-31, tuned 2026-09-01):** the whole haunting ladder, admin-scoped, in the `late-ssh/src/app/deadchannel/` game domain (subdomain `deadchannel/haunt/` owns onboarding; `glyphs.rs` holds the game's shared glyph alphabet). The ladder never skips a rung: stage 1, the sidebar clock glitch (~200ms, one or two glyph-alphabet characters, per-session dice, at most twice a UTC day, only while the clock is on screen; three persisted bursts quiet the clock and open stage 2). Stage 2, the own-name flicker on the landing echo of your own send (~1 in 24, one hit a day, three ever, ~800ms, two or three glyphs; grouped continuations are skipped since their author label never draws); the third hit arms stage 3 for the next fresh connect. Stage 3, the splash whisper (the held door): input acknowledged, never obeyed, the voiced line types itself, a ~10s hard cap always releases; delivery stamps `first_contact_whisper_at`. Stage 4, the invitation: the game's first voice (`afterglow`, a bartender-shaped ghost user, its row ensured at boot like `system` so the unique username index reserves the name) sends one persistent DM ending in `/join #deadchannel`; the invitation stamp is the key: `/join #deadchannel` (case-insensitive) joins the invited into the game's room (its own `kind='deadchannel'`, migration 170, seeded on first invited join, hidden from browse and IRC by kind whitelists) and gives everyone else the static line. All behind the process-global kill switch (`State.haunt_enabled`), armed only for admins; `/haunt` covers status/on/off plus per-stage test hooks. `late-ssh/src/app/deadchannel/CONTEXT.md`.)
- Status: Active
...
| `late-ssh/src/app/deadchannel/CONTEXT.md` | The deadchannel game domain (GAME.md): the first-contact haunting ladder (clock glitch, name flicker, splash whisper, invitation DM), the glyph alphabet, `/haunt` admin controls, or the haunt kill switch. | Domain layout (game root + `haunt/` onboarding subdomain), the four stage machines and their chain (hits arm the whisper, the whisper schedules the invitation), persisted `first_contact_*` marks, kill-switch semantics, the `afterglow` ghost voice, render hooks, `/haunt` command surface, tests, and gotchas. |
