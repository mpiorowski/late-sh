...
- Last updated: 2026-08-25 (chat gilds, the Shop's first chip sink that lives outside the Shop: `$` on a selected chat message opens a three-tier picker (Bronze 500 / Silver 5,000 / Gold 50,000, `late-core/src/models/chat_message_gild.rs`, migration 154). Two thirds of the price reaches the message's author as `ChipMove::GildReceived` and the last third is never re-minted: the burn is the gap between that reason and `GildSent`, and `GildReceived` is excluded from Top Chips so a tip is not a standing. Public rooms only, never your own message and never a bot's, one of each tier per buyer per message, and there is no un-gild. The permanent marker leads the message footer (`$$`, `$$$ x3`, in the tier's badge color) and crosses replicas over a `chat_message_gilded` Postgres notify rather than the in-process chat broadcast; the gild count per tier renders on the author's profile. #lounge hears about a message exactly once, on its third gild, and names only the author. `late-ssh/src/app/chat/CONTEXT.md` §9b.)
- Status: Active
