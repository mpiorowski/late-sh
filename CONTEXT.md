...
| PetCompanion | `pet_companions` | `user_id` UNIQUE, nullable `last_fed`/`last_watered`/`last_played` plus `last_treated` timestamp, `species` (`cat`/`dog`), and `care_streak_days`/`care_streak_date`; SSH pet care uses food every two days and daily water (the chase-toy play session was removed; `last_played` is a legacy column). Bought Pet Food can be used once per UTC day to treat the companion and start a 30-minute session-local roam. Mood is weighted: food hurts most, missing water is softer, and `Happy` requires all needs met for a 3-day completed-care streak; pets never die. |
