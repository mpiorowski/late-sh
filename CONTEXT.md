...
| Pot | `pots` | The weekly raffle (migration 160). `status` IN (`open`, `drawn`, `rolled`) with a partial unique index on `(status = 'open')` so at most one is open ever; `draws_at` is the next Monday 21:00 UTC, `ticket_price` 100. `winner_user_id`/`ticket_count`/`payout_chips`/`drawn_at` are all NULL while open and stamped once at the draw, enforced by one CHECK per status (a drawn pot is told from a rolled one by `ticket_count > 0`, not by the winner, whose account can be deleted out from under a settled row). `announced_threshold` is the high-water mark that makes the 50,000 / 100,000 #lounge lines once-per-pot across replicas and restarts. |
