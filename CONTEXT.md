...
- Last updated: 2026-07-29 (CodeKeep: The Pale added as a dedicated network door: lockfile-pinned `codekeep@1.0.9`, `late-codekeep` SSH/PTTY host, immutable-account HOME saves, and `Screen::Codekeep` + `HubGame::Codekeep`)
- Status: Active
...
| `late-ssh/src/app/door/codekeep/CONTEXT.md` | CodeKeep screen, npm/Bun package pinning, `late-codekeep` PTY host, account save identity, input forwarding, or deploy wiring. | CodeKeep client/host transport, immutable-account HOME saves, single-session lease, package integrity, config, infra, and tests. |
