...
- Last updated: 2026-08-24 (arrow keys in the doors, and a `/summary` floor. Every vt100-backed door now retypes cursor keys to the mode the game asked for (shared `late-ssh/src/app/door/keys.rs` gated on `Screen::application_cursor`; a no-op for games that never request it; invariant write-up in `late-ssh/src/app/door/dcss/CONTEXT.md` §4). NetHack needed a windowport change too: the default tty port reads raw `getchar()` and decodes no arrow escapes in any mode, so the `door-nethack` image (tag `5.0.0-r3`) now also compiles the curses windowport, a per-player opt-in via `OPTIONS=windowtype:curses` in the rc paste box; tty stays everyone's default (`late-ssh/src/app/door/nethack/CONTEXT.md` §6/§7). `/summary` now always covers at least the last 24h, widened by an older read marker and still capped at 48h / 200k chars: the read cursor advances just from sitting in a visible room, so an overnight-open terminal used to get "nothing new to summarize"; window policy lives in `summary::window_floor` (`late-ssh/src/app/chat/CONTEXT.md` §14).)
- Status: Active
