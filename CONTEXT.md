...
- Last updated: 2026-08-05 (Cyberspace integration v1: late.sh acts as a per-user personal client for cyberspace.online. New `cyberspace` synthetic rail entry in Home's Core section (linked accounts only, in both the navigation order and the rail render; unlinked users get the link modal over the room they are in, so nobody lands in a pane the rail does not list), `/cs` / `/cyberspace` composer commands (`post`, `link`, `unlink`), `late-ssh/src/app/chat/cyberspace/` slice (api/svc/state/input/ui), `cyberspace_accounts` table storing only the refresh token (migration 133), `ActivityKind::CyberspacePosted` #lounge line, and a hard AI blocklist for cyberspace.online URLs in the News summarizer. Their API terms ban bots/scraping/AI: every call runs under the linked user's own token on a human action, fetched content renders only for that user and is never cached server-side)
- Status: Active
