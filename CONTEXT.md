...
- **Adaptive world tick** — each render pass returns `App::wake_hint() -> Duration` (read under the app lock, after the draw) and the loop sleeps exactly that long unless input or a `RenderSignal` wake lands first. Three tiers (`app/tick.rs` consts): `HOT_TICK` 66ms while anything visible animates (splash, post-input 2s window, active ultimate effect, house tables, an open arcade game, pet, aquarium tray, bonsai modals), `AMBIENT_TICK` 266ms on the Clubhouse and while the audio visualizer animates on a visible sidebar, `IDLE_TICK` 500ms floor otherwise. Ticks at the floor only drain channels; an unprompted event (a chat message while idle) waits at most one floor interval. Advancing the world = `app.tick()`, render if dirty, ship the frame.
