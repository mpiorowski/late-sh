...
| `late-ssh/src/app/house/CONTEXT.md` | House tables (the fixed Poker/Blackjack/Asterion/Tron tables behind the Lobby modal), `Screen::HouseTable`, the four game runtimes, their singleton registry, seeded table chat/voice, or the backtick table stops. | `HouseTable` roster enum, `HouseTableRegistry` singletons + occupancy + seat activity, per-session `HouseState`/`HouseTableClient`, screen input/render split, runtime contracts for the four surviving games, and phase-3 demolition seams. |
| `late-ssh/src/app/rooms/CONTEXT.md` | Rooms screen, persistent game-room directory, embedded room chat, room creation/deletion, room shortcuts, or the dead room games (Chess/Tic-Tac-Toe/ssHattrick). | Sunsetting domain (phase-3 wholesale deletion pending): room service/persistence, directory input/rendering, manager traits and per-game managers/create modals, chat integration, timers, and room-game tests. The four surviving runtimes moved to `app/house/`. |
...
- `HouseTableRegistry` (in `app/house/registry.rs`) owns the process-global fixed house tables behind the Lobby modal: one lazy singleton Poker/Blackjack/Asterion/Tron service (no `game_rooms` rows), seeded permanent `chat_rooms(kind='game')` + voice channels per table, live occupancy for the modal, and the house `SatDown` activity choke point. Detailed contracts live in `late-ssh/src/app/house/CONTEXT.md`.
...
| DailyMatch | `daily_matches` | Correspondence games (chess/battleship/connect4), deliberately NOT a `game_rooms` row. Single table for challenge + match: `status` IN (`open`, `active`, `finished`, `cancelled`), `challenger_id`, nullable `opponent_id`/`target_user_id`/`turn_user_id`/`turn_deadline_at`/`winner_user_id`, `result` TEXT (`checkmate`/`draw`/`resign`/`timeout`/``''``), `state` JSONB (FEN, colors, move/position history, monotonic revision), nullable `chat_room_id` FK (migration 109: private two-player match chat + voice channel created in the claim transaction, reaped 30 days after finish/cancel). Claim is a guarded UPDATE; deadlines are DB timestamps swept by `DailyService`. See `late-ssh/src/app/daily/CONTEXT.md`. |
...
| `i`, `j`/`k`, message-action keys | Daily match board (players with match chat) | Compose in the embedded match chat; select messages; act on the selected message (reply/edit/delete/profile/copy/react) — same split as the active room. See `late-ssh/src/app/daily/CONTEXT.md`. |
| `` ` `` | Home / Daily match board | Hop Home chat → each daily match waiting on your move (nearest deadline first) → Home. Rooms and Arcade no longer participate (2026-07-13). |
