...
- Last updated: 2026-07-14 (activity ticker: #lounge system-feed lines no longer render as TUI chat rows — `ChatState` diverts them into a newest-first 10-item queue packed left to right into the one-row ticker in the composer-gap row of both Home chat surfaces (overflow collapses to `+N`); IRC projection and #lounge persistence unchanged; see `late-ssh/src/app/chat/CONTEXT.md`)
- Status: Active
