...
- Last updated: 2026-08-31 (first contact, stage 3, admin-scoped: the splash whisper, "the held door", from GAME.md's haunting ladder, in the new `late-ssh/src/app/haunt/` domain. An admin session whose `users.settings.first_contact_whisper_done` is unset arms a pure per-session state machine: the splash neither skips nor expires while the door is held, every keypress is acknowledged (deterministic static surge, the skip hint dissolving) but never obeyed, the voiced line types itself (in answer to the first keypress, or on its own at ~3s), and a hard cap (~10s) releases the door regardless of phase. Delivery spends the once-ever mark (fire-and-forget settings write); a kill-switch drop or lost session leaves it unspent. `/haunt` (parsed only for admins; a non-admin's `/haunt` posts as plain text so the command does not observably exist) prints status, flips the process-global in-memory kill switch (`State.haunt_enabled`, re-enabled by a restart, safe while admin-scoped), replays, and resets the mark. Render-only, TUI-only; stages 1-2 and the real eligibility campaign are unbuilt. `late-ssh/src/app/haunt/CONTEXT.md`.)
- Status: Active
...
| `late-ssh/src/app/haunt/CONTEXT.md` | First contact for deadchannel (GAME.md): the splash whisper ("the held door"), `/haunt` admin controls, the haunt kill switch, or any future haunting-ladder stage. | Whisper state machine and held-door contract, admin-only arming, the once-ever `first_contact_whisper_done` mark, kill-switch semantics, render helpers, `/haunt` command surface, tests, and gotchas. |
