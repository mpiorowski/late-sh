...
- Support workloads are placed on `agent-1` by the 2026-09-13 infra change: service-web, Icecast, Liquidsoap, otel-collector, vmagent, kube-state-metrics, VictoriaMetrics/Logs/Traces, Grafana, Minecraft. `server-1` keeps service-ssh, redis, the doors, ingress-nginx, ipv6-proxy, LiveKit, and both Postgres instances, the last two by decision. See Immediate Next Work 3
...
**Status 2026-09-13: node joined; support workloads moved in Terraform, effective with the next infra release.** Postgres and LiveKit deliberately stay on `server-1`. What landed, what stayed and why, and what is left:
...
**Volumes are recreated empty, not migrated, by decision.** Downtime and lost history were accepted in exchange for zero migration machinery. A `local-path` PVC is pinned to the node holding it, so a selector alone would leave the pod unschedulable. Release procedure: scale `liquidsoap`, `grafana`, `victoriametrics`, `victorialogs`, and `victoriatraces` to 0, delete `music-data`, `grafana-data`, and the three Victoria PVCs, confirm they are gone, cut the infra release (the apply recreates them on `agent-1`), then dispatch `sync_music`. Deleting a PVC while its pod still runs leaves it Terminating and breaks the apply. Consequences:

- Radio streams silence from the scale-down until `sync_music` finishes: `radio.liq` has no fallback beyond the files on the PVC, and R2 is the source of truth.
...
- Grafana loses UI-made state and its admin password resets to the Terraform secret. Dashboards and datasources are provisioned from config, and the VictoriaMetrics plugins reinstall at boot (the 508 MB `plugins` directory was most of the old volume).
- `server-1` gets back roughly 8 GiB of volume data.
...
**Minecraft addressing.** Its hostPort opens on `agent-1`, but `late.sh` and the `*.late.sh` wildcard resolve to `server-1`, so two manual DNS records route players: an `mc.late.sh` A record for `agent-1`, and a `_minecraft._tcp.late.sh` SRV record so plain `late.sh` still works. Both DNS-only, never proxied.
...
- Record a post-move baseline for both nodes once traffic settles.
