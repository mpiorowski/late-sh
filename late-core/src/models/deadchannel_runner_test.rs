use crate::models::deadchannel_runner::DeadchannelRunner;
...
async fn ensure_creates_once_and_keeps_the_first_look() {
    ...
    let created = DeadchannelRunner::ensure_for_user(&client, user.id, &first_look)
        .await
        .expect("ensure");
    ...
    // the first look, the same id.
    let second_look = serde_json::json!({"hood": "hood.plain"});
    let again = DeadchannelRunner::ensure_for_user(&client, user.id, &second_look)
        .await
        .expect("ensure again");
