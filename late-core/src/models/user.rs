                 ...
                 LEFT JOIN LATERAL (
                    ...
                    WHERE pa.user_id = u.id
                      ...
                      AND (
                        ...
                        OR pa.category IN ('lateania_archdemon', 'lateania_frontier_king', 'lateania_sundering_deep', 'lateania_kaethyr_ascendant', 'nethack_amulet', 'nethack_ascension', 'dcss_orb', 'dcss_win', 'greendragon_dragon')
                      )
                 ) award ON true
                 WHERE u.id = ANY($1)",
                &[
...
fn chat_profile_award_badges(raw: Option<String>) -> Option<String> {
    ...
    // Archdemon, an Ascension implies the Amulet, and a DCSS escape implies
    // the Orb pickup. Profile views still show all; chat author labels show
    // only the highest.
