...
async fn watering_is_scoped_to_the_owner() {
    ...
    let owner = User::create(
        &client,
        UserParams {
            fingerprint: "bonsai-scope-owner".to_string(),
            username: "bonsai-scope-owner".to_string(),
            settings: serde_json::json!({}),
        },
    )
    .await
    ...
    let other = User::create(
        &client,
        UserParams {
            fingerprint: "bonsai-scope-other".to_string(),
            username: "bonsai-scope-other".to_string(),
            settings: serde_json::json!({}),
        },
    )
    .await
    ...
    assert_eq!(other_after.growth_points, 0);
}
