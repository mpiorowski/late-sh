...
impl RssFeedRead {
    pub async fn mark_read_now(client: &Client, user_id: Uuid) -> Result<()> {
        client
            .execute(
                "INSERT INTO rss_feed_reads (user_id, last_read_at, updated)
    ...
    pub async fn last_read_at(client: &Client, user_id: Uuid) -> Result<Option<DateTime<Utc>>> {
        let row = client
            .query_opt(
                "SELECT last_read_at FROM rss_feed_reads WHERE user_id = $1",
                &[&user_id],
            )
            .await?;
    ...
    pub async fn unread_count_for_user(client: &Client, user_id: Uuid) -> Result<i64> {
        let row = client
            .query_one(
                "SELECT COUNT(e.id)::bigint AS unread_count
                 FROM rss_entries e
                 LEFT JOIN rss_feed_reads rfr ON rfr.user_id = $1
                 WHERE e.user_id = $1
                   AND e.shared_at IS NULL
                   AND e.dismissed_at IS NULL
                   AND (
                     rfr.user_id IS NULL
                     OR e.created > COALESCE(rfr.last_read_at, '-infinity'::timestamptz)
                   )",
                &[&user_id],
            )
            .await?;
