...
use super::user::{
    ...
    extract_keep_composer_focused, extract_show_settings_on_connect, extract_terminal,
...
impl Profile {
    ...
    /// into settings via `settings || jsonb_build_object(...)`, so concurrent
    /// writes to unrelated keys (ignored_user_ids) are preserved.
