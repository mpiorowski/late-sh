...
roster!(
    ...
    /// daily game here enrolls it in all four surfaces at once.
...
/// One query for every per-puzzle win-count board in the window: the union
/// arms carry the puzzle key and ranks partition by it.
async fn fetch_daily_win_boards(
    client: &Client,
    window: DailyWindow,
) -> Result<HashMap<DailyPuzzle, Vec<RankedEntry>>> {
    let date_filter = match window {
        DailyWindow::Monthly => format!("WHERE puzzle_date >= {MONTH_DATE_FILTER}"),
        DailyWindow::AllTime => String::new(),
    };
    let arms: String = DailyPuzzle::ALL
        .iter()
        .map(|puzzle| {
            format!(
                "SELECT user_id, '{key}' AS game FROM {table} {date_filter}",
                key = puzzle.key(),
                table = puzzle.wins_table(),
            )
        })
