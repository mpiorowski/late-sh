...
#[allow(dead_code)]
impl Ppu {
    pub fn new(mapper: Rc<RefCell<Mapper>>, cart_mirroring: Mirroring, frame: RenderFrame) -> Ppu {
        Ppu {
    ...
    pub fn tick(&mut self, ppu_cycles_to_tick: usize) -> TickEvent {
        ...
        for _ in 0..ppu_cycles_to_tick {
            match self.state.next(self.rendering_enabled) {
                ...
                (Phase::PreRender, 280..=304, Rendering::Enabled) => {
                    self.copy_vertical_from_t_to_v()
                }
