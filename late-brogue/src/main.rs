...
// curses binary on a PTY and serves it over SSH; late-ssh connects as a client
// and proxies the terminal into its Brogue launcher (the same transport as the
// dcss and nethack hosts).
//
...
async fn main() -> anyhow::Result<()> {
    ...
    // SIGHUP-saves in-flight games (our brogue build saves-and-exits on hangup
    // via the Dockerfile-applied patch) instead of losing them to SIGKILL.
    ...
    tokio::select! {
        ...
        _ = wait_for_shutdown_signal() => {
            tracing::info!("shutdown signal received; SIGHUP-saving live brogue children");
            let _ = shutdown_tx.send(true);
            // Hold the process open long enough for the bridges to run brogue's
            // hangup-save. Exceeds host.rs's per-child HANGUP_SAVE_GRACE and must
            // stay under the pod's terminationGracePeriodSeconds (service-brogue.tf).
...
/// Mirrors `late_core::shutdown::wait_for_shutdown_signal`; late-brogue has no
