...
pub struct ClientHandler {
    shared: Arc<Shared>,
    /// Sanitized playname from the authenticated SSH username; keys the
    /// per-player save directory (the child's cwd).
...
/// The TERM to hand the brogue child: the client's own if the host can resolve
/// its terminfo, otherwise a universal fallback. ncurses aborts on an
