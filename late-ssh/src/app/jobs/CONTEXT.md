...
`sources_test.rs` (the parsers over `fixtures/`), `state_test.rs` (tags, ranking, the match line, `/jobs`), `post_test.rs` (the form's rows and rules), `svc_test.rs` (the due day, the drip arithmetic, the excerpt cut, a release reaching the shelf through the snapshot and `/jobs`, the admin gate, a shelf write landing, the per-person cap, and the take-down against a real DB), `late-core/src/models/job_posting_test.rs` (upsert, settle, the attempt cap, the slice, expiry, the daily claim, post and retract), `directory/ui_test.rs` (the shelf drawn), `paper/state_test.rs` (NEW WORK).
