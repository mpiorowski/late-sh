...
- Read: one `generate_json` (schema enforced, `AI_MODEL`) per `pending` row, at most `JOBS_READ_LIMIT` = 400 a run; the model decides remote only for HN. A failed read counts an attempt and waits for the next night; at `JOBS_MAX_READ_ATTEMPTS` = 3 the row is tombstoned. One `HEAD` at the link through the SSRF guard (`files::image_upload::head_url`: private and reserved addresses refused before any request, DNS pinned, no redirects followed): 404, 410, no host, or a refused address is `dead`; everything else keeps the link. `settle` only touches a `pending` row, so a run that overlaps another (a `/jobs pull` during the nightly run) cannot pull a released row back into the queue. Each read settles and shelves on its own (`settle_read`): an `active` row joins this replica's snapshot at once, so the shelf fills while the run is still reading and a run that fails later keeps what it read. Other replicas see it at their next refresh.
