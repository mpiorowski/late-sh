// Create-room modal for Lateania. The world has no configurable options in the
// slice, so this is a single name field, mirroring the Tic-Tac-Toe modal.
...
impl MudCreateModal {
    ...
    fn submit(&mut self) -> CreateModalAction {
        ...
        if display_name.is_empty() {
            self.error = Some("World name is required.".to_string());
...
impl CreateRoomModal for MudCreateModal {
    fn draw(&self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_widget(Paragraph::new(section_heading("World")), layout[1]);
        frame.render_widget(
...
fn footer_line() -> Line<'static> {
    Line::from(vec![
        Span::raw("  "),
        Span::styled("\u{21B5}", Style::default().fg(theme::AMBER_DIM())),
