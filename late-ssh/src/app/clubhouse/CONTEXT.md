...
| `input.rs` | Walking (arrows/hjkl), `i` composer, `w`/`x` emotes, `t` bartender mention, `n` out back to Nightcap, Enter on landmarks/dog/the back door, tutorial Enter. Returns `false` for globals. |
