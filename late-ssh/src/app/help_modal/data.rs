...
pub(crate) fn bot_app_context() -> String {
    let mut out = String::from(
        ...
        - The Games hub (page 3) is the dedicated landing for the door games Lateania, NetHack, DCSS, Brogue, Usurper, Green Dragon, A Dark Room, dopewars, CodeKeep, BashQuest, and Rebels; each is launched from there, not from its own top-level page. A Dark Room is the odd one out: it is an incremental, so it grows on its own while you are connected to late.sh (about three hours of village time a day, wherever you are in the app) instead of being played in one sitting. It is also the only door with an ending, and it has two of them: flying the starship out pays 10,000 chips and the [ADE] badge, and doing it while carrying the fleet beacon taken off the immortal wanderer on the ravaged battleship pays another 10,000 and the [ADB] badge. Each is once per account and they are claimed separately, so one account can earn both. Either way the save is wiped and the room starts dark again. The battleship itself only appears on the map once the account has finished the game at least once, so a first run never meets it.\n\
...
fn chips_help_lines() -> Vec<String> {
    ...
    vec![
        ...
        "              (the Archdemon Mal'gareth,".to_string(),
        "               the King Who Was Promised Nothing,".to_string(),
        ...
        "  A Custom Title (2,000 / 24h, 60,000 / 30d) is a title you write yourself, up to 20 characters.".to_string(),
...
fn arcade_help_lines() -> Vec<String> {
    [
        ...
        "  [ADB]     A Dark Room Homefleet          10,000 chips",
    ]
    .into_iter()
    .map(str::to_string)
    .collect()
...
fn overview_lines() -> Vec<String> {
    [
        ...
        "  custom title      write your own, 20 characters, screened before you are charged",
