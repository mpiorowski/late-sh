...
fn arcade_help_lines() -> Vec<String> {
    [
        ...
        "  #puzzle-art: post one square image URL or upload, optionally with a title. /rules has limits. Staff f then 1 approves it into the daily puzzle pool; other reactions do nothing.",
        "  Sliding Puzzle: image tiles are the default. i toggles numbered and image tiles; your choice is saved per user (same board and rewards).",
        "  Loading or failed art falls back to numbered tiles without changing your preference. Press i twice to retry failed art.",
        "  One shared image from the approved pool rotates daily at UTC midnight across all difficulties; an open board keeps its art. Personal art follows the saved board.",
