...
- The modal sits above every other overlay in input and render order; a ready paper waits in `pending_modal` while a newcomer's tour holds the keys. `/paper preview` carries today's announcements so far the same way (`PressOutcome::Previewed.announcements`).
