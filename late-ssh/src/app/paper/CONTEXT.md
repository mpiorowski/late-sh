...
- Sections: `reading` (yesterday's News shares via `Article::list_shared_between`, `quiet` when nobody shared) and `outside` (grounded `generate_reply`; a `NOTHING` answer settles `quiet`). `outside` prints only while `paper_outside_enabled` is on.
- Switches (`app_flags`, closed `AppFlag` enum): `paper_enabled` (kill switch, seeded on) and `paper_outside_enabled` (seeded off). `/paper on|off` and `/paper outside on|off` flip them, admin only, through `AppFlagService::set_task` like `/haunt`.
...
- The session arms `login_pop_pending` for returning users with the `paper_at_login` tweak on (Ctrl+O Tweaks → Startup → "Daily paper at login", default on); `tick` fires it once the splash is down. A new user's first session never pops.
