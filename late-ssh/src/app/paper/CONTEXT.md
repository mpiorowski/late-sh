...
- Layout (`lay_out`): byline, ANNOUNCEMENTS (each post as `@author · HH:MM` then its body lines as written, no markdown pass; the section is absent on a day the operator said nothing), NEW WORK (the job feed's releases of the covered day, read at open time with no claim and no model, printed for every reader while `jobs_enabled` is on: a reader with an open or casual work card gets up to `PAPER_MATCHES` = 3 matches on their card's tags and langs, or a pointer at the shelf when none matched, a reader with no card gets the one line that sells the card, and a not-looking card or a day with no release gets a one-line faint hint; `PaperIssue.work`, built by `read_work`, is `None` only with the switch off, which drops the section; see `late-ssh/src/app/jobs/CONTEXT.md`), YOUR ROOMS in rail order (favorites first, from `ChatState::visual_order`), ELSEWHERE ON LATE.SH (public rooms you are not in, bumped rooms first, top `PAPER_ELSEWHERE_LIMIT` = 3, with a `/join` hint on topic rooms only, since `/join #<code>` would open a new topic room rather than the language room), WHAT WE WERE READING, OUTSIDE, then a footer naming quiet rooms, rooms still at the press, and rooms that missed it (`failed` at the cap). Nothing in the layout is per reader beyond ordering and membership.
