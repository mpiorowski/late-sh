...
- Last updated: 2026-08-08 (the log pipe ships, Phase 2 of `devdocs/PLAN-ROGUELIKE-BOARDS.md`: the vt100 screen scrape is deleted; boards, the NHA/NHY badge pair, and death/win feed events now come from the host's xlogfile + livelog over a reserved `late_stats` SSH session, mirroring DCSS)
- Status: Active
...
- Client `state.rs`: `connect` no-op when disabled; `forward_input` without a proxy is a no-op; `strip_input_noise` drops mouse/paste but keeps keys/arrows; F1 (both encodings) consumed; exit-grace opens on close and counts down; idle shutdown closes a stale running game.
