...
- Last updated: 2026-08-05 (per-account .nethackrc paste box: DB-stored rc pushed at every launch over an SSH env request, host materializes it and points NETHACKOPTIONS at it)
- Status: Active
...
- `infra/nethack.tf`: the RWO `nethack-save` PVC (`local-path`, 2Gi, `prevent_destroy`) + the host/port locals. The PVC + seed initContainer **moved here from `service-ssh`**; `service-ssh.tf` now only injects `LATE_NETHACK_SECRET` (host/port live in the config.rs prod profile).
...
- CI: `.github/workflows/deploy_nethack.yml` builds and rolls out nethack, and only nethack, for `-nethack` releases: `ci` + `build`, then `kubectl set image` on the `late-nethack` deployment (both the `nethack-save-seed` init container and the main container) and a rollout wait. No terraform runs on ordinary releases, so nothing else in the cluster is touched. When the `late-nethack` deployment is missing the run auto-bootstraps (a terraform apply `-target`-scoped to the nethack resources only); other manifest changes go through `deploy_infra.yml`; other deploy workflows that do run terraform read the live tag off the `late-nethack` deployment and pass it through the required Terraform input, so an ordinary release never rebuilds or restarts the door. Same rule as codekeep, dcss, brogue, dopewars, and usurper. `nethack.yml` build-validates `docker/doors/nethack.Dockerfile` and publishes the pinned `door-nethack` image on main pushes. License/source obligations tracked in `NOTICE` (NGPL).
...
- Milestone reward scraping is best-effort, not authoritative. A marker must be the WHOLE pline leading row 0 in `milestone.rs`: it starts the line AND is followed only by nothing, the terminal `--More--`, or NetHack's two-space message concatenation. The whole-pline rule exists because pet names lead their own plines (rc `DOGNAME=` or the in-game C-call), so a start-of-line anchor alone was spoofable; pet plines continue after a single space and name munging collapses double spaces, so they cannot fake the allowed endings. The once-per-account guarantee comes from the lifetime reward template + `NOT EXISTS` award insert in `award.rs` (plus per-session debounce in `state.rs`). Residual risk is accepted (cosmetic flair, not a competitive economy): a text channel that produced a pline whose entire sentence *is* a marker would still spoof; none is known. The only spoof-proof source is NetHack's host-side xlog/logfile, which would need a cross-crate signal — deliberately not built. Match on the exact NetHack 5.0.0 strings (re-verify on any version bump).
