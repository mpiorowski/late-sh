...
- **Feed events (no reward, no persistence).** The same screen scrape also posts visible activity-feed events (`ActivityKind::GameEvent`, category `Game`) for non-reward moments: launching ("started a NetHack game", on `connect`), reaching a new deepest dungeon level (parsed from the `Dlvl:` status field), and dying. These are flavor only — no chips, no badges, nothing written to the DB beyond the in-memory activity stream. Depth uses a state comparison (deepest-seen) rather than counting events; death matching is zero-false-positive (quit/save/ascension excluded).
...
| `milestone.rs` | Pure screen-text matchers for the Amulet pickup, ascension, and death messages (verified against NetHack 5.0.0 source). Ascension requires a *prelude* line to have been seen first, so a single spoofed string can't trigger the payout; death matches are zero-false-positive (never fire on quit/save/ascend). |
