...
- Last updated: 2026-08-05 (per-account init.txt paste box: DB-stored rc pushed at every launch over an SSH env request, host materializes it and passes crawl's `-rc`)
- Status: Active
...
- **The `-macro` flag alone does NOT survive a player rc.** Our build is non-DGL, so a pushed rc can set `macro_dir` (also `save_dir`/`morgue_dir`), and crawl's `-macro` only seeds SysEnv *before* the rc is read (verified against the pinned 0.34.1 `initfile.cc`: CLO_MACRO vs the macro_dir GameOption; `-name` by contrast IS re-applied after the rc, so rc `name =` cannot open another player's save). The host therefore appends a final `-extra-opt-last macro_dir=<per-player dir>` (`host.rs::crawl_args`, test-pinned): opt-last lines are processed after the whole rc including its Lua, so the host keeps the last word and a pushed rc cannot plant macros in another player's dir. rc `save_dir`/`morgue_dir` repointing is accepted: it only moves the player's own name-keyed data around an already-shared-UID volume. Keep the guard as the LAST opt-last argument when touching `crawl_args`.
