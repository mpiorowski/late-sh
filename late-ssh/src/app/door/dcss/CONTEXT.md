...
- Last updated: 2026-08-07 (the log pipe ships: a reserved `late_stats` SSH session on the host streams crawl's logfile/milestones to late-ssh's ingestion task, which lands `door_runs`/`door_milestones`, grants the DCO/DCW badge pair, and feeds the DCSS boards on the Leaderboards page; the build gains `DGL_MILESTONES`/`TIME_FN=gmtime`/`DGL_EXTENDED_LOGFILES`)
- Status: Active
...
- **The log pipe (boards, badges, feed events — no screen scraping).** crawl appends machine-readable xlog files under `$HOME/.crawl` on the host's PVC: `logfile` (one line per finished game, written unconditionally by upstream) and `milestones` (live mid-run events; exists only because the build defines `DGL_MILESTONES`, see §3). The host serves them over a reserved SSH username, `late_stats` (`late-dcss/src/stats.rs`; inside the reserved `late_*` namespace so no player can claim it): the client pushes per-file byte offsets in one env request (`LATE_DOOR_STATS_CURSORS`), the host streams `<file-id>\t<offset>\t<line>` frames with tail -f semantics and stays stateless — no cursor storage, no parsing, no DB. late-ssh's ingestion task (`app/door/ingest/`: one connect-with-retry loop from main.rs, gated on `LATE_DCSS_ENABLED`; `dcss.rs` pure parsers verified against the pinned 0.34.1 source; cursors in `door_log_cursors`) lands one `door_runs` row per finished game and `door_milestones` rows for orb/rune/branch-entry milestones, idempotent via unique `(game, source_file, source_offset)`. The playname on every line is the arcade handle; reserved `late`/`late_*` shapes and dead handles are skipped. From this pipe: the DCSS board triple on the Leaderboards page (Wins all-time, Deepest dive, Top score), the lifetime badge pair `DCO` (Orb pickup, 10k chips, granted at pickup from the milestone stream) and `DCW` (escape with the Orb, 20k, back-grants DCO), and #lounge feed events for deaths, the Orb pickup, and wins (freshness+recency gated so backfill never floods the feed). Sprint/seeded games write suffixed files (`logfile-sprint`) the pipe deliberately does not stream, and explore/wizard games are never logged upstream.
...
| `stats.rs` | The `late_stats` log-streaming session: cursor env parsing, complete-line framing (`frame_lines`), and the stateless tail loop over `logfile`/`milestones`. Constants mirrored in late-ssh's `app/door/ingest/stream.rs`. |
...
- The build passes `EXTERNAL_DEFINES="-DDGL_MILESTONES -DTIME_FN=gmtime -DDGL_EXTENDED_LOGFILES"` — three dgamelaunch-server behaviors the plain build leaves off (normally implied by `DGAMELAUNCH`, which we deliberately do NOT define wholesale). `DGL_MILESTONES` makes `mark_milestone` write the milestones file at all (verified against the pinned source: without it the file simply never exists), `TIME_FN=gmtime` puts the xlog timestamps in UTC (localtime otherwise), and `DGL_EXTENDED_LOGFILES` adds the preformatted `tmsg` death one-liner the feed events use. Each is asserted fail-closed via the `-version` CFLAGS grep, same as `NOWIZARD`. Bumping the recipe bumps the `door-dcss` image tag in the root Dockerfile and `dcss.yml` (`0.34.1-r2` for this change).
...
Inline pure tests cover: `late-core` `models::arcade_handle` (shape + reserved rules), client+host `identity.rs` (derivation determinism + distinctness + the cross-crate known-answer fingerprint pinning `late.sh/dcss/v1`, mirroring nethack's), client `state.rs` (disabled connect no-op, forward without proxy no-op, mouse/paste stripping, F1 both encodings, exit grace, claim-prompt byte handling/caps/validation errors), host `playname.rs` (sanitize), host `server.rs` (`effective_term` fallback), host `stats.rs` (cursor parsing, line framing), hub `state.rs` (card order incl. DCSS).

The log pipe's tests live with the ingestion slice: `app/door/ingest/dcss_test.rs` (pure parsers against realistic 0.34.1 lines incl. hostile cases), `stream_test.rs` (listener smoke test of the stats client against a stub SSH host, patterned on the rebels proxy test), `svc_test.rs` (DB-backed: replay idempotency, reserved/dead-name skipping, once-per-lifetime badge grants, cursor advancement), and the board queries in `late-core/src/models/leaderboard_test.rs`.
...
- **HTTP publishing for dcss-stats.com/Sequell** (Phase 4 of the plan): serve `logfile`, `milestones`, and `morgue/` read-only over HTTP from the pod so the public tooling can ingest late.sh as a server. Coordinate URL shapes with the maintainer first; their fetcher and our ingestion would then validate each other off the same files.
...
- Rune-count and branch boards: the pipe already persists `rune` and `br_enter` milestones, so those boards are a query away when wanted.
