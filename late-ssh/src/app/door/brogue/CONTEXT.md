...
- Last updated: 2026-07-28 (save-isolation audit recorded in §4, pinned upstream source copied to `upstream-brogue/`)
- Status: Active
...
- **On any Brogue CE version bump, re-run both upstream audits in the same commit:** the hangup patch (§1/§4) and the save-isolation checks (§4), then re-pin `upstream-brogue/` (§3). Both are properties of upstream source that no test of ours covers, so a bump can silently void either one.
...
- **The cwd is the only wall between players, and it holds because of an upstream input filter.** Every child runs as the same uid in one pod on one shared PVC: no chroot, no per-player uid, no seccomp, so `current_dir(&player)` in `host.rs` is the entire isolation story. It is sufficient today only because brogue gives a player no way to name a path (audited against the pinned source in `upstream-brogue/`, 2026-07-28):
  ...
  **Re-audit all four on a CE bump**, next to the hangup-patch check. Nothing in our test suite fails if upstream relaxes that filter or adds a path-taking feature; the wall would just be gone.
...
- **Harden the isolation instead of inheriting it.** Per §4, player separation currently rests on an upstream input filter rather than on anything we enforce. Running each child under a per-player uid, or giving it only the player dir as a writable mount, would make the boundary ours and survive a CE bump that relaxes the filter. Not urgent while the audit holds, but it is the difference between a checked invariant and an enforced one.
