...
- **The hangup-save is ours.** Upstream's curses build dies unsaved on SIGHUP (only the SDL window-close path auto-saves). The Dockerfile `brogue-build` stage applies `scripts/brogue_hangup_save.patch`, which installs a SIGHUP/SIGTERM handler running upstream's own prompt-free `quitImmediately()` save path (flush recording, write the suspend `.broguesave`, `_exit`). Verified by starting a game on a PTY, SIGHUPing, and finding the save on disk. Everything downstream (the host's SIGHUP-then-SIGKILL teardown, the pod's SHUTDOWN_GRACE drain) relies on this patch; re-verify it on Brogue CE version bumps.
...
- The child is spawned with `env_clear()` + allowlist (`TERM` via the host's `effective_term` fallback, `HOME` = the player dir). No locale vars: the curses build renders pure ASCII (`glyphToAscii`, plain `-lncurses`). `LINES`/`COLUMNS` are deliberately NOT exported (ncurses would freeze at spawn-time geometry); the pty winsize (openpty + TIOCSWINSZ on `window_change`) is the only size source. No CLI args at all: players cannot pass flags, so wizard mode (`--wizard`, a runtime flag in brogue, not a compile knob) is unreachable.
...
| `render.rs` | Ratatui rendering: `draw_landing`/`draw_launcher` (BROGUE logo, blurb, dungeon strip, hints) and `draw_running` which blits the live `vt100` screen via `rebels::render::blit_screen`. In-game help is brogue's own `?`. |
...
- **Awards from the per-player run history, not the screen.** brogue appends a machine-readable line per game to each player dir's run history file (result, depth, seed, score, "Escaped the Dungeons of Doom!" wins). An award pipe (chips/badges for an escape, a lumenstone, a deep dive) should read those host-side and signal late-ssh rather than scraping vt100. Needs the same cross-crate signal path DCSS §9 defers.
