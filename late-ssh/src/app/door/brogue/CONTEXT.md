...
- **Fixed grid, centered, letterbox reset.** Brogue's grid is a fixed 100x34; it announces it with one `ESC[8;34;100t` at startup, which vt100 drops by default, so `proxy.rs::HonorResize` implements the resize callback and shrinks the parser to match. `render.rs::grid_rect` then centers the grid in the viewport (pinning top-left when the viewport is smaller); `clear_letterbox` resets the whole door area first so the surround shares one terminal-default canvas with the keyed-out interior instead of framing the game in leftover page background.
