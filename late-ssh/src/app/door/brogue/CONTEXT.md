...
- Last updated: 2026-08-04 (detach-and-resume: ` steps out with the game live, 20-minute input-idle shutdown, backtick-cycle/hub resume)
- Status: Active
...
- **On any Brogue CE version bump**, re-pin `upstream-brogue/` (§3) and bump the `door-brogue` image tag. The two upstream properties we depend on (the hangup patch and the §4 save isolation) are asserted fail-closed in the build itself, so a bump that voids either one fails the image build rather than shipping; run it locally against the new source first with `scripts/brogue_verify_isolation.sh upstream-brogue`.
...
- Compiled in the `brogue-build` stage (`docker/doors/brogue.Dockerfile`, published as the ghcr `door-brogue` image the root Dockerfile pins) from the pinned GitHub tag tarball, SHA-256 verified fail-closed (`BROGUE_*` ARGs). Build: `make bin/brogue TERMINAL=YES GRAPHICS=NO RELEASE=YES` (curses-only, no SDL; RELEASE drops the `-dev` version suffix that brogue bakes into save names). One source patch, `scripts/brogue_hangup_save.patch` (see §1), applied with a fail-closed grep; the stage also runs `scripts/brogue_verify_isolation.sh` (the §4 save-isolation assertions, ~0.2s) and asserts `--version` prints exactly `Brogue version: CE <version>`. Both greps and the script gate the `make`, so a source that fails either never produces a binary.
...
- CI: `.github/workflows/deploy_brogue.yml` builds and rolls out brogue, and only brogue, for `-brogue` releases: `ci` + `build`, then `kubectl set image` on the `late-brogue` deployment (both the `brogue-save-seed` init container and the main container) and a rollout wait. No terraform runs on ordinary releases, so nothing else in the cluster is touched. When the `late-brogue` deployment is missing the run auto-bootstraps (a terraform apply `-target`-scoped to the brogue resources only); other manifest changes go through `deploy_infra.yml`; other deploy workflows that do run terraform read the live tag off the `late-brogue` deployment and pass it through the required Terraform input, so an ordinary release never rebuilds or restarts the door. Same rule as codekeep, nethack, dcss, dopewars, and usurper. `brogue.yml` (PR/weekly build-validate of `docker/doors/brogue.Dockerfile`, publishes the pinned `door-brogue` image on main pushes, manual verify_deployed). License obligations tracked in `NOTICE`: **AGPL-3.0, and because we run a patched build, section 13 requires the corresponding source (pinned tarball + our patch + build recipe) to stay publicly available while the door is hosted**; this repo is that offer.
...
- **Identity is asserted by late-ssh, not proven to the host.** The shared secret authorizes the *connection*; the playname is merely the SSH username, so anyone who can both reach the host and hold the secret can open any player's saves. That makes two things load-bearing rather than incidental: `late-brogue-sv` stays a ClusterIP off ingress and the ssh-tcp LoadBalancer (`infra/service-brogue.tf`), and a leak of `LATE_BROGUE_SECRET` is a total identity bypass, so rotate it on both sides together.
