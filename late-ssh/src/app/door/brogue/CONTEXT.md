...
- **Fixed grid, centered, letterbox reset.** Brogue's grid is a fixed 100x34 (it emits `ESC[8;34;100t` at startup, which the vt100 crate honors by resizing the parser to match), so the viewport is usually larger than the game. `render.rs::grid_rect` centers the grid; `clear_letterbox` resets the whole door area first, because the app root paints every page over `BG_CANVAS` and `blit_screen` never touches cells outside the grid: without the reset the game floats in a page-colored frame instead of sharing one terminal-default canvas with its keyed-out interior.
