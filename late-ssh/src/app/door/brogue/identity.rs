...
const KEY_DOMAIN: &[u8] = b"late.sh/brogue/v1\0brogue\0";
...
/// playname that keys the per-player save directory). The server accepts
/// exactly this one derived public key; both ends recompute it from
/// `LATE_BROGUE_SECRET`.
pub fn derive_client_key(secret: &str) -> PrivateKey {
    ...
    PrivateKey::new(KeypairData::from(kp), "late.sh brogue derived").expect("valid ed25519 key")
}

// CROSS-CRATE CONTRACT: `KEY_DOMAIN` and every derivation step above MUST stay
// byte-identical to late-brogue's `identity::derive_client_key`. If they drift,
// the client derives a different key and the host rejects every connection,
// so the contract is pinned by the known-answer test in identity_test.rs,
// which must match the identical KAT in late-brogue.
