...
pub struct StateConfig {
    ...
    pub enabled: bool,
    /// Render-loop wakeup (from the transport). `None` on headless/test paths.
    pub repaint: Option<Arc<RenderSignal>>,
    /// Accessor for the account's arcade handle. `None` on headless/test paths.
    pub handle_svc: Option<ArcadeHandleService>,
}
...
impl State {
    ...
    pub fn tick(&mut self) {
        if self.mode == Mode::Running {
            ...
            if closed {
                ...
                // brogue's death recap or high-score screens, and those trailing
    ...
    /// prompt, retry). Keeps the Brogue screen up while no game is running;
    ...
    pub fn proxy(&self) -> Option<&BrogueProcess> {
        self.proxy.as_ref()
    }

    /// Intercept the F1 key before it reaches brogue. Returns true when the
    /// input was consumed and must NOT be forwarded as-is.
    ///
    /// F1 is remapped to brogue's own `?` help menu: it is the conventional
    ...
    /// Forward client bytes to brogue, minus mouse and bracketed-paste reports.
    /// brogue's curses build does take ncurses mouse input, but that expects
    /// the game itself to own the tracking mode; late.sh keeps any-event mouse
...
/// Drop terminal reports brogue must never see: SGR mouse (`ESC [ < … M/m`),
