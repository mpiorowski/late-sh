...
/// The late-usurper host is a trusted, late.sh-owned service reached over the
/// internal network. We accept any server host key and rely on the derived
/// shared-secret credentials for auth (same policy as the nethack/dcss doors).
...
/// Per-session proxy to the late-usurper SSH host. Owns a background task that
/// runs the bidirectional bridge; the foreground holds a shared vt100 screen
/// and a status flag updated by that task.
///
/// This is the twin of the dcss door's `DcssProcess`: same vt100 model and
/// transport, but the target is late.sh's Usurper host and the SSH username
/// carries the account's arcade handle as the dropfile identity. The remote
/// bytes arrive already transcoded to UTF-8 (the host owns the CP437 mapping).
pub struct UsurperProcess {
...
pub struct ProcessConfig {
    ...
    /// re-sanitizes it and writes it into the session's DOOR32.SYS, which keys
    /// the game's player record. Claimed once and immutable
    ...
    pub fn spawn(cfg: ProcessConfig) -> Self {
        ...
        let task = tokio::spawn(async move {
            if let Err(e) = run_bridge(cfg, cmd_rx, task_parser, task_status.clone()).await {
                tracing::warn!(error = ?e, "usurper proxy bridge ended with error");
            }
...
async fn run_bridge(
    ...
    .await
    .context("usurper outbound connect timed out")?
    ...
    // account's arcade handle (the host writes it into the dropfile).
    let key =
    ...
    .await
    .context("usurper outbound authenticate_publickey timed out")?
    .context("outbound authenticate_publickey failed")?;
    if !auth.success() {
        anyhow::bail!("usurper host rejected derived credentials");
    }

    let mut outbound = timeout(SETUP_TIMEOUT, session.channel_open_session())
        .await
        .context("usurper outbound channel_open_session timed out")?
    ...
    .await
    .context("usurper outbound request_pty timed out")?
    .context("request_pty failed")?;
    timeout(SETUP_TIMEOUT, outbound.request_shell(true))
        .await
        .context("usurper outbound request_shell timed out")?
