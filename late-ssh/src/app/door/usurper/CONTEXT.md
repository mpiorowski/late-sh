...
- Last updated: 2026-07-20 (initial build of the door)
- Status: Active
...
- While Running, raw client bytes are forwarded to the child, minus mouse/paste noise **and all function keys F1-F12** (`state.rs::strip_input_noise`). In DOOR32 local mode the player's keyboard IS the game's sysop console, and DDPlus binds F2 (sysop chat), F7/F8 (time adjust), F10 (eject), letting those through would hand every player the sysop panel. There is **no F1 remap** (the game has no universal help key; its menus list their own keys).
...
| `host.rs` | `PtyHost`: the per-session PTY bridge, openpty + env_clear + setsid/TIOCSCTTY + XON/XOFF clear + detached reader, CP437→UTF-8 on output, ASCII filter on input, `StopReason` teardown (SIGHUP grace then SIGKILL). |
...
### Images / infra / CI
- `runtime-usurper` stage: `/opt/usurper` (bins + seed) + `/var/lib/late-usurper` chowned to `late`. `base` gets the same tree for `dev-usurper` (compose `service-usurper`).
...
- **Strip F1-F12 client-side.** In local mode the player's keys are the sysop console keys (F2 chat, F7/F8 time, F10 eject). Re-check the strip if DDPlus keybindings change on a version bump.
