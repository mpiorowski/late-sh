...
| `graduate.rs` | `BashquestAwards`: thin `Db`-backed sink that persists a verified graduation (mirrors `nethack::award::NethackAwards`'s fire-and-forget shape). `None` on headless/test paths. |
...
- `late-web/src/pages/bashquest_graduates/bashquest_graduates_test.rs`: the `BashquestGraduate` → `GraduatePayload` mapping carries only the intended public fields.
...
Publishes a verified "this account graduated BashQuest" fact to a public gallery outside late.sh entirely (`github.com/hardlygospel/bashquest-graduates`, GitHub Pages), while making sure nothing in the chain can be spoofed by a player who only ever gets PTY keystrokes into the running game, never a shell or filesystem access.

**Chain of custody, host → client → database → public site:**

1. **Host (`late-bashquest/src/host.rs`):** right before closing the channel in `run_bridge` (after the child has already exited — see `report_certificate_if_new`), check whether bashquest.sh wrote `$data_dir/.bashquest/<playname>.certificate.txt` (its own `graduation_ceremony`, only reached after completing all 61 levels) and whether a sibling `.certificate.reported` sentinel is absent. If so, send a marker **directly via `handle.data()`** — never anything read from the child's PTY output — then touch the sentinel so a graduate logging back in doesn't resend it every session.
...
5. **`door::bashquest::graduate::BashquestAwards::spawn_record`** (mirrors `nethack::award::NethackAwards`'s fire-and-forget shape) persists it via `late_core::models::bashquest_graduate::BashquestGraduate::record`, an `INSERT ... ON CONFLICT (user_id) DO NOTHING` into the `bashquest_graduates` table (migration `142_create_bashquest_graduates.sql`) — first record per account wins and is immutable; a re-report is a harmless no-op.
6. **`late-web`'s `GET /api/bashquest/graduates`** (`late-web/src/pages/bashquest_graduates/`) is a public, read-only JSON mirror of that table — the first DB-backed JSON handler in `late-web` (existing pages are either HTML-only or proxy late-ssh's own internal API; see that module's doc comment).
7. **`github.com/hardlygospel/bashquest-graduates`** (separate repo) runs a scheduled Action that pulls the *entire* current list from that endpoint and fully mirrors it — never an append-only merge — into `data/graduates.json` plus one generated page per graduate. That repo's own README carries the same chain-of-custody explanation for a reader who lands there without this context.

**Why each hop matters:** a player has no shell, so they cannot write the certificate file themselves (step 1's precondition). The marker cannot come from anywhere except host-originated Rust code (step 1/2). The client cross-checks the digest before trusting content (step 3). The database write is keyed off the session's real authenticated account, not the self-reported handle string (step 4/5). The public site only ever mirrors the database, never accepts a submission (step 6/7). Breaking the guarantee requires compromising late-bashquest's host process itself, not just knowing how the format works.
