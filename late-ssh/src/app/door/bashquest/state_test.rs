...
#[test]
fn disabled_door_never_awaits_a_handle() {
    // A disabled door's HandleFlow is built with `svc: None`, so it settles
    // straight to `Missing` (no lookup in flight) instead of hanging in
    // `Loading` forever.
    let state = disabled_state();
