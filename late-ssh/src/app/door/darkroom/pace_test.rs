...
use super::pace::{DAILY_CREDIT_SECS, Pace, SLOWDOWN, slowed};
...
fn daily_allowance_truncates_a_long_session() {
    ...
    let first = pace.grant(now, 2 * 60 * 60);
    let second = pace.grant(now, 6 * 60 * 60);
