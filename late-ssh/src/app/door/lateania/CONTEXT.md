...
- Last updated: 2026-08-15 (second player-feedback pass: quest journal rows and inventory/shop rows now show real description text instead of a bare name+stats; the bounty board is a picker menu (`Panel::Board`) instead of examine silently auto-assigning the next static-list bounty - see the Board quests section and §9's note on the exhausted single-letter keyspace; poisons moved out of `Valuable` into a new `ItemKind::Utility`, splitting the "Consumables" category into "Heals" vs a general "Consumables" so batch-sell can never wipe a buff item out from under a player; the map's gather markers now carry the node's real level requirement; villager interaction dropped the "you ask X for a moment" preamble that read as an unfulfilled action. Backtick joins the active-world keys: it detaches onto the backtick workspace cycle - a full autosaved leave plus a 5-minute recency window that keeps Lateania a cycle stop for a one-key rejoin of the same slot; see §3 Input capture contract and `lobby/CONTEXT.md`)
- Status: Active
