...
- Last updated: 2026-08-22 (the live map cut stays at the atlas region (`live_filter_region`/`in_viewed_region`, unchanged from the follow-up fix pass), but the snapshot's `nearby_foes`/`nearby_players` lists are now scoped by `in_zone_neighbourhood` (player's zone plus zones one exit away, read off the room graph via `ZONE_LINKS`) instead of the raw cell window alone: the hand-authored core embeds into a box 41 rows tall, so the window around one capital contained four others and painted their live mobs onto whichever city the player stood in. Scenery scope and dagger scope are deliberately different questions; spec in §5.1's "A harder cut on top" bullet)
- Status: Active
