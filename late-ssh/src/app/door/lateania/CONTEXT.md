...
- `Inventory`: pack items plus equipped items as rows. Enter on a row is context-sensitive via `state::inv_action`: worn gear comes **off** (`unequip_task`), loose gear goes on, a consumable is used. Selling checks the `equipped` row specifically, not the item id: a worn item refuses with a reason, but a loose duplicate of that same item id sells fine while the worn copy stays on (`svc::sell`). **On terminals ≥ 100x20 it expands to a full-screen master-detail layout** (`draw_inventory_screen`, the shop screen's twin): one line a piece (name, sell value, the `▲/▼` tag or `worn`) under the collapsible headers on the left, and on the right the highlighted piece against what is worn in its slot (`InvView::worn_name`/`worn_stats`), its description, what Enter does to it (`inv_action`), and whether `x` sells it here. The sell keys are only offered as live when `view.shop` is present. Both screens build their rows and detail through the shared `item_row_line`/`item_detail_lines`, so they cannot drift apart. The sidebar `inventory_panel` serves anything smaller.
