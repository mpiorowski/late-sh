...
- The landing's character list is a cache (`LateaniaService::character_slots`) filled by the fire-and-forget `character_slots_task` (on `set_screen(Lateania)`, on leaving the world, and after a delete). The render loop only paints on input or a reported change, so that task takes `App::repaint_signal` and wakes the loop once the list lands, the same pattern as the Arcade's handle loads; without it the landing showed five empty slots (or a just-deleted character) until the next keypress. Known gap: the delete path fires two refreshes (`leave_lateania` then the delete task's own), so a very late first fetch could briefly resurrect the deleted row until the next refresh.
