...
- **Eighteen lines of static keybinds trailed every room** (`footer_hints`), duplicating the `?` guide. What is specific to the room now **leads** the panel instead: `room_actions` renders a `You can` block right under the vitals, in a louder style (`action_hint`, green key + bright label), for the keys that only work where you stand (`g` raise, `b` shop, `p` stable, `u` craft, `y` gather, `q` tame, `i` ways, `n` ledger, `o` board, `~` feed). Empty in a room with none of them. `footer_hints` is now four dim compact lines of the standing keys plus `? all keys`; mid-fight it is just the four combat keys. Anything trimmed from it was added to the `?` guide (`help_modal/data.rs::lateania_help_lines`), which is the one place the full reference lives.
...
- Keep the Archipelago's ID ranges aligned: 20 islands, rooms at `20000+` (`archipelago::ARCH_BASE`/`ARCH_STRIDE`), mob IDs at `970000..980000` (`ARCH_SPAWN_ID_START`, `Band::Archipelago`, its own dedicated row rather than sharing Kaelmyr's), generated gear at `5000..5200` (`ARCHIPELAGO_ITEM_BASE`/`ARCHIPELAGO_TIERS`, `power_offset = 60`), and regional finds at `3800..3840` (`ARCHIPELAGO_FIND_BASE`). **Island tier is `isle + 52`, deliberately above Kaelmyr's `z + 32..51` band** - never lower, since that is exactly the bug this port fixed (Kaelmyr's own zones out-xp'd/out-golded every Archipelago island; gold is derived straight from xp via `gold_for_kill`, so an xp regression silently regresses gold too) and it is also what "The lands" table in §7 has always claimed the Archipelago's level actually is. `archipelago_boss_loot` weights the two find ids into the pool at increasing odds by quartile (islands `0..10` ×1, `10..15` ×2, `15..20` ×4), and the last three islands (`isle + 3 >= ISLAND_COUNT`) are `apex`: +25% boss hp/xp, +15% boss damage. **The Archipelago's bosses are not crowns** - they are ordinary named bosses like every zone boss elsewhere, not in `CROWNS`/`CROWN_TARGETS`; nothing here needs the three-places rule above.
- Keep the Thornveil Falls ID ranges aligned: 12 zones, rooms at `34000+` (12×8 = 96 per zone, `THORNVEIL_BASE`/`THORNVEIL_ZONE_STRIDE`), mob IDs at `1000000+` (`THORNVEIL_SPAWN_ID_START`, well clear of Aelunor's `1600000+` and Wildbound's `1500000+`), classified by `band_of` into its own `Band::Thornveil` - a distinct band, but tuned to the **identical row** as `Band::Kaelmyr` in `tune_spawn_balance`'s match (both the main hp/dmg/xp table and the regular-mob respawn-rate `endgame` check just below it - a new band added to one and not the other silently splits their treatment). No generated gear catalog - `thornveil_loot`/`thornveil_notable_loot` borrow `reaches_loot` at an offset tier plus Thornveil's own regional finds at `3900..3924` (`THORNVEIL_FIND_BASE`). **Thornveil is maze-only** (`thornveil_zone_is_cavern` always `false`), unlike every other mixed continent, because `every_public_safe_room_has_a_villager` needs every zone entrance room id known exactly for the static `VILLAGERS` table, and a `carve_cavern` entrance cell is RNG-picked rather than fixed at cell 0 like a maze's - adding cavern zones later means deriving those entrance ids from an actual `seed_world()` run, not guessing them. Tier is `z + 30`, overlapping Kaelmyr's *lower* half by design - a parallel track, not a harder/easier clone, so don't push it past Kaelmyr's own ceiling or it collides with the Archipelago's role as the top of the curve. **Not a crown**: its final boss is an ordinary named boss, not in `CROWNS`/`CROWN_TARGETS` - if it should become one, that needs real arena calibration (§10's battle arena section), not a guessed number. When changing Thornveil Falls, update `THORNVEIL_ZONES_DATA`, `is_thornveil_room`, the `REGIONS` atlas entry, the room-count band, and the shape/reachability tests together.
