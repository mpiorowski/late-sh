...
**The map is never in the side rail.** With RPG mode on and 96+ columns, the Room panel is the live field + room rail + full-width log strip. Narrower than that, and with RPG mode off, the classic log + side view stands in; its rail carries the room summary only, never a minimap (the old bottom-of-rail minimap is gone, `PlayerView` carries none). `m` opens the overhead map.
