...
- Last updated: 2026-08-23 (the coordinate field no longer folds: the Wildbound Waste decodes as reserved blocks via `wildbound_layout`, each descent zone has its own z-level, and the town/road wings were re-aimed, all pinned by `zone_interleaves` + `no_zone_presses_against_another_it_has_no_gate_into`. The live region filter, the `exempt` sets, and the `in_zone_neighbourhood` foe-list scoping that compensated for the folds are removed; the `nearby_foes`/`nearby_players` lists are scoped by the cell window alone again. `link` asserts instead of silently skipping an occupied exit, and `every_walkable_room_is_reachable_from_the_start_room` pins connectivity)
- Status: Active
