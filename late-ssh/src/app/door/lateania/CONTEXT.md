...
- Last updated: 2026-09-17 (any destination is trackable at any distance: the green map arrow aims along the real walk via `worldmap::track_aim`, stopping at the stair or land-crossing that leaves the player's floor/land; see the compass paragraph in §5. `G` feeds your own companion anywhere; loyalty is capped at `pets::MEALS_PER_DAY` meals a UTC day, and a well pet with no meal to have, meals spent or max level, is turned away free; see "Combat companions" in §7. The inventory has the shop's full-screen master-detail layout, `draw_inventory_screen`, with vitals and the fight on its title line; see `Inventory` under "Panels" in §5. An ability's killing blow reaches the next tick's output: `tick` drains `pending_kills` and never clears it up front.)
- Status: Active
