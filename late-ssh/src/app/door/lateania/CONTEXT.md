...
- World actions: `r` recalls to Embergate's Town Square when out of combat; `f` toggles the Follow panel; `g` casts the Resurrection rite on the nearest fallen adventurer in the room (Cleric/Paladin/Druid only); `p` opens the Stable (companion vendor) where one stands; `n` opens the housing ledger (at the clerk, or inside a home you own); `e` opens the appearance/bio builder; `y` opens "the Ways" fast-travel menu when standing on a waystone portal; `m` toggles the **World Atlas** (a whole-world exploration overview: `World::region_progress` scores each of the `REGIONS` for visited/total rooms + boss count, rendered as meters + `◆N` loot markers by `ui.rs::atlas_panel`). In the Inventory panel `A`/`C`/`J` batch-sell all loose gear / commons / non-upgrades (keeping potions and worn gear); inventory and shop rows show a coloured `▲+N%` / `▼-N%` upgrade tag vs. what's worn.
