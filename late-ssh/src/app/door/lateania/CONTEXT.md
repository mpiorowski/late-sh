...
- Last updated: 2026-08-13 (second player-feedback pass: quest journal rows and inventory/shop rows now show real description text instead of a bare name+stats; the bounty board is a picker menu (`Panel::Board`) instead of examine silently auto-assigning the next static-list bounty - see the Board quests section and §9's note on the exhausted single-letter keyspace; poisons moved out of `Valuable` into a new `ItemKind::Utility`, splitting the "Consumables" category into "Heals" vs a general "Consumables" so batch-sell can never wipe a buff item out from under a player; the map's gather markers now carry the node's real level requirement; villager interaction dropped the "you ask X for a moment" preamble that read as an unfulfilled action. Earlier, 2026-08-10 (navigation pass: scattered-link stubs are sided by their exit's direction rather than by a meaningless cross-block coordinate delta, which had the map drawing paths that were not there; the map now draws stairs at all (it hid every zone-to-zone exit), names where a zone sits in its region's chain, and can be given a destination with `x` for a turn-by-turn `heading` line - see §5.1, which also records the measurement those three answer. Slot/pvp correctness pass: character saves now resolve their slot from the *live* character (`live_slot`), not the landing's account-wide `active_slot` - a second connection selecting another slot used to overwrite the character saved there; taking a mob target now breaks off a duel, so `target`/`pvp_target` can never both be set - see §6 pvp and §8 Character slots. Earlier player-feedback pass: Esc no longer closes the game while chatting and now requires a confirming second press to actually leave; the Mend boon heals fully; the day/night clock got a glyph + danger colour; stray-feeding messages spell out the real UTC-midnight countdown; the Leaderboard moved off `?`, which is globally reserved - see §9 Critical Invariants; selling a loose duplicate of worn gear no longer requires unequipping first; accounts can now keep up to 5 character slots - see §8 Character slots; overhead map collisions now favour the player's own region instead of an arbitrary lowest id, known-but-scattered links get their own brighter stub distinct from plain fog, and the footer legend is a fuller four lines - see §5.1)
- Status: Active
...
- **Off-screen POI border arrows point in a meaningless direction once the POI is in another reserved block, and are unfiltered.** `worldmap::poi_arrows` walks all ~162 boss/tameable POIs on the player's z-level with no distance, fog, or selection filter, so an 80x24 map opened in Embergate draws ~17 of them. Showing unexplored ones is *intended* (the doc comment: a "that way" hint, not a spoiler), and the volume is survivable. The real flaw is the same one `Tile::HintKnown` had: the arrow's direction comes from the coordinate delta, which across reserved blocks records only which block `derive_coords` laid down first, not where the place is. So a Broceliande boss reads as "east" for no reason, and an arrow whose whole job is pointing the way does not point the way. The clean fix when someone picks this up is a `PAN_LIMIT` (400 cell) cutoff rather than an arbitrary one: `COMPONENT_MARGIN` is 800, so that keeps exactly the same-block POIs whose direction is a real spatial relationship, and drops the ones the camera could never pan to anyway. Left as-is on purpose for now - noted so the next person doesn't rediscover it from scratch.
