...
- `infra/doors.tf` (`codekeep` entry, stamped out by the `infra/door` module) owns the 1Gi RWO `codekeep-save` PVC, the one-replica kill-before-create Deployment, and the internal Service on 2328. The init container chowns the mounted root to `late`.
- `-codekeep` releases route through `release.yml` to the generic `deploy_service.yml`: `ci` + `build`, then `kubectl set image deployment/late-codekeep '*=<image>'` (covers the `codekeep-save-seed` init container and the main container) and a rollout wait. No terraform runs on ordinary releases, so nothing else in the cluster is touched; when the `late-codekeep` deployment is missing the run auto-bootstraps (a terraform apply `-target=module.door["codekeep"]`). Other manifest changes go through `deploy_infra.yml`; terraform never reads or rewrites live image tags (deployments carry `ignore_changes` on images), so an ordinary release never rebuilds or restarts the door. Same rule as every other door.
