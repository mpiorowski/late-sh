...
/// Per-session proxy to the dedicated late-codekeep host.
pub struct CodekeepProcess {
    ...
    pub fn spawn(cfg: ProcessConfig) -> Self {
        ...
        let task = tokio::spawn(async move {
            if let Err(e) = run_bridge(cfg, cmd_rx, task_parser, task_status.clone()).await {
                tracing::warn!(error = ?e, "codekeep proxy bridge ended with error");
            }
...
impl Drop for CodekeepProcess {
    fn drop(&mut self) {
        self.task.abort();
    }
}

/// Stable, opaque account label. The host validates this exact shape and uses it
/// as the per-account HOME directory name, so CodeKeep saves survive reconnects
/// without exposing mutable usernames to the storage contract.
pub fn codekeep_session_label(user_id: uuid::Uuid) -> String {
...
async fn run_bridge(
    ...
    .await
    .context("codekeep outbound connect timed out")?
    .with_context(|| format!("connecting to {}:{}", cfg.host, cfg.port))?;

    let username = codekeep_session_label(cfg.user_id);
    let key =
    ...
    .await
    .context("codekeep outbound authenticate_publickey timed out")?
    .context("outbound authenticate_publickey failed")?;
    if !auth.success() {
        anyhow::bail!("codekeep host rejected derived credentials");
    }

    let mut outbound = timeout(SETUP_TIMEOUT, session.channel_open_session())
        .await
        .context("codekeep outbound channel_open_session timed out")?
    ...
    .await
    .context("codekeep outbound request_pty timed out")?
    .context("request_pty failed")?;
    timeout(SETUP_TIMEOUT, outbound.request_shell(true))
        .await
        .context("codekeep outbound request_shell timed out")?
        ...
        tokio::select! {
            cmd = cmd_rx.recv() => match cmd {
                ...
                Some(OutboundCommand::Resize { cols, rows }) => {
                    let _ = outbound.window_change(cols as u32, rows as u32, 0, 0).await;
                }
                None => break,
            },
            msg = outbound.wait() => {
