...
- Last updated: 2026-09-04 (the `artboard` gallery award joins the monthly snapshot: ranked `ART1`-`ART3` by each hanger's best piece's applause, `GALLERY_AWARD_MIN_APPLAUSE` = 3 to rank at all, and the only ranked award that pays chips, 10k / 5k / 1k, paid inside the snapshot transaction off the `RETURNING` rows so a re-run or a second replica pays nothing. `snapshot_previous_month_profile_awards` now takes `&mut Client` and returns `AwardSnapshotOutcome`.)
