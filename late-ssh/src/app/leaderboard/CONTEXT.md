...
- Last updated: 2026-09-06 (Top Chips counts everything in `chip_ledger` on both sides except blackjack, poker (bets, payouts, floor restores) and gifts; gilds, the pot, spends, Super Snake, the bonsai drip and the starting stipend all count. Admin `/grant` chips never reach the ledger. The rule is one sentence on `ChipMove::counts_as_earnings`.)
...
- `Top Chips` (monthly sum over `chip_ledger` of the moves whose `ChipMove::counts_as_earnings` is true, exclusions derived from `ChipMove::excluded_earning_reasons()`: everything counts on both sides except blackjack, poker (bets, payouts, floor restores) and gifts, so a colluding table cannot fold one seat to the top and nobody can be funnelled up; gilds, the pot, spends, Super Snake, the bonsai drip and the starting stipend count as they land, and admin `/grant` chips never reach the ledger; the rule and its reasoning live on `counts_as_earnings` in `late-core/src/models/chips.rs`) and `Arcade Wins` are bespoke monthly-only boards. Arcade Wins weights come from `Difficulty::points` (easy/draw-1 = 1, medium = 3, hard/draw-3 = 5; Sliding Puzzle uses all three tiers, Le Word fixed Easy, Rubik's fixed Medium), the same enum whose `chips()` carries the daily-win payout tiers, so points and payouts cannot drift apart. Unknown difficulty keys score 0, never a default.
