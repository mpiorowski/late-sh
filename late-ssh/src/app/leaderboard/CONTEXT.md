...
- Last updated: 2026-09-01 (the profile award snapshot loop is rollover-aware: it ticks hourly but writes only at startup, when the previous UTC month changed since its last successful pass, or on a 24h fallback. The old fixed 24h interval was anchored to process start, so freshly-completed monthly badges could stay blank for up to a day after the 1st while the chat-label query already filtered on the new previous month. A failed pass now retries on the next hourly tick instead of waiting out the fallback. Gate: `svc.rs::should_snapshot_awards`)
