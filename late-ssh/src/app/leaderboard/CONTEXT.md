...
- Last updated: 2026-09-04 (the `artboard` gallery award joins the monthly snapshot: ranked `ART1`-`ART3` by each hanger's best piece's applause, `GALLERY_AWARD_MIN_APPLAUSE` = 3 to rank at all, and the only ranked award that pays chips, 10k / 5k / 1k, paid inside the snapshot transaction off the `RETURNING` rows. Its arm is settled once per month (`NOT EXISTS` on the period's `artboard` rows, since applause keeps moving after the rollover) and ranked with `ROW_NUMBER` over applause then earliest hang (a tie must not pay two first prizes), so a re-run or a second replica pays nothing whatever the applause did since. `snapshot_previous_month_profile_awards` now takes `&mut Client` and returns `AwardSnapshotOutcome`.)
