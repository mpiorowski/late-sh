...
- Last updated: 2026-09-07 (Top Chips ranks what a player earned: every credit except the house tables (blackjack, poker, floor restores), gifts and the starting stipend; a debit never counts, so no spend (the Shop, the bar, the pot, the crown, a gild, an arena loss) lowers anyone. Gilds received and the pot count. Admin `/grant` chips never reach the ledger. The rule lives on `ChipMove::counts_as_earnings`.)
