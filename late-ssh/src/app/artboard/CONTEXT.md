...
- `late-ssh/src/app/artboard/gallery/` (the gallery subdomain, same file roles)
  ...
  - `svc.rs`: `GalleryService` (db + `app_flags` watch + the process-wide splash wall `watch`), the spawned tasks (`list_task`, `hang_task`, `applaud_task`) reporting `GalleryResult` over the session channel, `refresh_splash` / `start_splash_refresh_task` (assigns and publishes the day's piece, none while the switch is off), `claim_splash_piece` (the per-login claim, see §Gallery). Owns the gallery's logs and metrics.
...
- The splash. The wall: every piece hung takes one UTC day over the door, in hang order, the day after it was hung at the earliest. `ArtboardPiece::splash_for_day` (migration 193) reads the piece stamped `splash_on` for the day or, when none holds it yet, stamps the oldest piece never shown and hung before the day; the partial unique index over `splash_on` (pieces still up) is the whole concurrency story, the loser of a race reads the winner back, and a mod removal after the day was assigned leaves the day empty (the cup), nothing promoted into the gap. The gallery's switch is in both statements, so a day the gallery was off assigns nothing and burns no piece. It sits in the process-wide `GalleryService` splash `watch` as `Option<SplashPiece>` (the piece and its `shown_on` day), refreshed hourly by `refresh_splash`, which is also the assignment: the first replica awake on a day stamps it, the rest read it back, and each refresh logs `queued` (`ArtboardPiece::splash_queue_depth`, pieces still waiting for a day) so the backlog is measurable before anyone decides on a cap. Each login claims its account's one view (`GalleryService::claim_splash_piece` from both session builders, `session_bootstrap.rs` and `ssh.rs`, into `SessionConfig::splash_piece`): `User::claim_splash_shown(day)` is one `UPDATE` on `users.settings.splash_shown_on`, one key overwritten in place, keyed on the piece's own day rather than the clock so a watch still on yesterday's piece for an hour past midnight shows it only to accounts that missed it and never spends today's view; the first login of the day shows the piece, every later login the coffee cup. The claim is made before the terminal size is known, so a too-small terminal spends the view on the cup (`draw_splash_piece` returns false), the same way the paper's stamp is spent before its modal draws. The caption says how long before its day the piece was hung (`hung yesterday`, `hung 3 days ago`). The switch off, no database, a failed claim, or an empty queue are all the cup. Last month's podium (`ART1`-`ART3`) is no longer on the splash; the award rows and the hall of fame keep it.
