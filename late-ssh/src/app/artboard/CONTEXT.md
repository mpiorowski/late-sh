...
Month end (`late-core/src/models/profile_award.rs`): the `artboard` category ranks hangers by their best piece's applause over their pieces of the month (`period_month`, the UTC month hung), only pieces at or over `GALLERY_AWARD_MIN_APPLAUSE` (3). The rank is `ROW_NUMBER` over applause then earliest hang, never `RANK`: this is the one arm that mints chips, and a tie must not pay two first prizes; the tiebreak is the one the hall of fame uses, and `previous_month_podium` reads these rows back, so the splash's podium is `ART1`-`ART3` in order. Ranks 1-3 print `ART1`-`ART3` and pay `gallery_prize_chips` (40,000 / 15,000 / 10,000) as `ChipMove::ArtboardPrize` inside the snapshot transaction, keyed off the insert's `RETURNING` rows. **The month is settled once**: this is the arm that pays, so it must complete exactly once however many passes run (the 24h fallback, a restart, another replica). `toggle_applause` closes the month at the rollover, but a mod removal still moves the ranking afterwards, and `ON CONFLICT DO NOTHING` alone would let a hanger who climbed into the top 3 on a later pass get a fresh row and a fresh prize; the arm carries `NOT EXISTS (artboard row for the period)` and ranks nobody once any row exists. Applause is therefore counted once, by the first pass within the hour after the rollover.

Where a piece shows up beyond the page:
- The splash. Last month's podium (`ArtboardPiece::previous_month_podium`, a `Vec<PodiumPiece>` of place and piece) is read off the minted award rows: the `artboard` `profile_awards` rows for the month, rank `<= GALLERY_PODIUM_SIZE` = 3, each joined to its hanger's best piece still up (`applause DESC, created ASC`, the award's own order, so it is the winning piece unless a mod took that one down). The place travels with the award row, so a mod removal after the month settled leaves a gap (or hangs the hanger's next piece) rather than promoting whoever stood behind, and the podium is empty until the award pass has run (within the hour after the rollover; the hourly refresh picks it up). It sits in the process-wide `GalleryService` splash `watch` as `Vec<SplashPiece>`, refreshed hourly. Each login claims the account's next entry (`GalleryService::claim_splash_piece` from both session builders, `session_bootstrap.rs` and `ssh.rs`, into `SessionConfig::splash_piece`): `User::claim_splash_podium_slot` is one `UPDATE ... RETURNING` on `users.settings.splash_podium = {"month", "shown"}`, one key overwritten in place, counting entries shown, not ranks, so the first login of a month shows the first entry (`1st of <month>` on an intact podium), the next the second, and every login after the last entry the coffee cup until the podium changes. A podium shorter than three stops sooner; a stored month past the podium's (a replica behind the calendar) neither resets nor counts. The claim is made before the terminal size is known, so a too-small terminal spends the entry on the cup (`draw_splash_piece` returns false), the same way the paper's stamp is spent before its modal draws. The switch off, no database, a failed claim, or an empty podium are all the cup.
