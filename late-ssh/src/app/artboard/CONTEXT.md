...
- `late-ssh/src/app/artboard/gallery/` (the gallery subdomain, same file roles)
  ...
  - `svc.rs`: `GalleryService` (db + `app_flags` watch + the process-wide splash `watch`), the spawned tasks (`list_task`, `hang_task`, `applaud_task`) reporting `GalleryResult` over the session channel, `refresh_splash` / `start_splash_refresh_task`, `wall_piece_for` (the paper). Owns the gallery's logs and metrics.
...
- `late-ssh/src/app/artboard/page.rs`
  ...
  - View mode supports cursor movement, page/home/end, Alt-arrow panning, right-drag pan, `?` local help, `g` to the rail, and `i`/Enter activation.
...
The rail (view mode): `Board` (the live board or the archive being viewed), a GALLERY group (`This month`, `New`, `Hall of fame`, `Mine`), `Hang a piece`, then an ARCHIVES group last (`Daily`, `Monthly`, `Curated`). The gallery rows and `Hang a piece` exist only while `artboard_gallery_enabled` is on. Edit mode hides the rail. Under `MIN_WIDTH_FOR_RAIL` = 64 columns the rail shows only while it has focus (`g` brings it up, Enter or Esc on Board puts it away). The page lands on the rail with `Board` selected; Enter (or `l`, Right) hands the keys to the board cursor.

Focus (`gallery::state::Focus`): `Rail` (the landing), `Canvas` (the board cursor), `List` (a listing's pieces), `Piece` (one piece full frame), `Archive` (an archive kind's key list drawn in the rail's place). `g` on the board focuses the rail; Enter on a gallery row moves into its list; Enter on an archive row turns the rail into that kind's list; Esc walks back out; Esc on the rail returns to the board; Tab backs out of a pane to the rail (on the rail and the board Tab stays the page switch). Arrow keys follow focus. Mouse: a click on a rail row selects and activates it, a click on a list row selects it, the wheel scrolls the list.

Archives from the rail: the list shows keys only (one summary query per kind per session, newest first, the count on the rail row); the key under the cursor loads in the background and replaces the board as it lands (`Mode` reads `snapshot`, the Board row's tail reads `archive`, the list marks the loading key with `…` and the one on the board with `•`). Enter on a key moves focus to the board with the archive up; Esc goes back to the rail rows and leaves the archive up; the Board row returns to live.
...
Month end (`late-core/src/models/profile_award.rs`): the `artboard` category ranks hangers by `MAX(applause)` over their pieces of the month (`period_month`, the UTC month hung), `HAVING >= GALLERY_AWARD_MIN_APPLAUSE` (3). Ranks 1-3 print `ART1`-`ART3` and pay `gallery_prize_chips` (10,000 / 5,000 / 1,000) as `ChipMove::ArtboardPrize` inside the snapshot transaction, keyed off the insert's `RETURNING` rows; a re-run or another replica inserts nothing and so pays nothing. Applause is counted when the pass runs, within the hour after the rollover.
...
| The rail | landing, `g` | `j/k` or arrows move, `Enter` opens (Board, a listing, an archive list, hang), `Esc` to the board, `Tab` from a pane back to the rail; on a narrow terminal the rail shows only while focused |
...
- The rail shifts the board 21 columns right in view mode; anything that computes a board cell from a screen point must go through `canvas_area_for_state` with the last draw's rail visibility, never `canvas_area_for_screen`.
