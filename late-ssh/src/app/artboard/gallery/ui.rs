...
/// The splash: the day's wall piece over the door, if it fits under the
...
pub fn draw_splash_piece(frame: &mut Frame, area: Rect, splash: &SplashPiece) -> bool {
    ...
    let caption = Line::from(vec![
        Span::styled(
            format!("{}: ", hung_label(splash.shown_on, piece.created)),
            Style::default().fg(theme::TEXT_FAINT()),
        ),
        Span::styled(
            format!("\"{}\"", piece.title),
...
/// How long before its day over the door a piece was hung, as the splash
/// caption says it: "hung yesterday", "hung 3 days ago". A piece is never
/// shown the day it was hung, so "today" does not occur; a clock behind
/// the piece reads as yesterday rather than a negative count.
fn hung_label(shown_on: chrono::NaiveDate, created: chrono::DateTime<chrono::Utc>) -> String {
    let days = (shown_on - created.date_naive()).num_days();
    match days {
        ..=1 => "hung yesterday".to_string(),
        n => format!("hung {n} days ago"),
    }
}
