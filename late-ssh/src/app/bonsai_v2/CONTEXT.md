...
- `natural_steps`: a free tip (not wired, not the trunk) meanders around its heading with a seeded roll per step: keep going, one notch toward level, one notch toward up, and at order 4+ sometimes a droop. Young orders mostly climb, deeper orders drift level and hang. The rolled step comes first and the others follow as fallbacks when a cell is taken, so a taken cell costs the wave nothing. A step that would leave the pot is never taken: the wall parks the tip (which `tip_can_grow` then excludes from the bud gate, a ceiling tip outright) and the tree's energy goes to buds, which prefer room. Nothing slides along the pot.
