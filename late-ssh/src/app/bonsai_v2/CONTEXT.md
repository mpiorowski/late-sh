...
- `deflect_at_pot`: a tip whose step would leave the canvas slides along the wall instead (level along the ceiling, upward or level-away at a side wall). Only the pot deflects; a neighbouring branch does not, so a crowded tip waits rather than wriggling.
