...
- Last updated: 2026-07-11 (Embedded webview split out of the `late` binary on Linux: the webview code moved to the new `late-webview` crate, shipped as a standalone helper binary next to `late`, so `late` no longer links WebKitGTK/GTK and starts fine on hosts without the webview libraries — installing the distro webkit/gstreamer packages later enables embedded YouTube without reinstalling. Windows/macOS keep the in-process `late webview-pair` path via the same crate as a library. See §17.)
- Status: Active
