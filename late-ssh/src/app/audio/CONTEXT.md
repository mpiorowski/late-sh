...
- **Embedded webview initial seek only.** On helper open, `late-cli/src/webview/pair.rs` consumes the first `queue_update.current.started_at_ms` snapshot and applies a one-shot `startSeconds` to the first matching `load_video`. The flag is consumed by that first load, even on mismatch, so server heartbeats and later queue track switches keep the normal no-offset behavior.
