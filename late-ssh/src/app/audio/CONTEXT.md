...
- Last updated: 2026-08-15 (Mute and volume are now persisted per device in `user_ssh_keys.settings` and are the one source of truth for both: `m` and `+`/`-` stick across sessions, a mid-session pair-WS reconnect no longer unmutes anyone, and the `Start app with music muted` settings row is gone. See "Mute and volume: one source of truth, stored per device")
- Status: Active
