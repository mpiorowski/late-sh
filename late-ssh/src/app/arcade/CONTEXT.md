...
- Last updated: 2026-09-08 (Share cards: every finished daily has a result card, `s` copies it to the clipboard, the lobby's `s` copies the day card, and the lobby lists the dailies first. Sliding Puzzle's session-only image view encodes opaque native Kitty/iTerm2/Sixel images once per board cell per source/geometry/protocol. Moves rearrange those immutable cells and emit only the changed placements, Kitty replacing before its targeted delete; the encoded set and the downloaded source bytes are cached for the session and freed when the board is no longer open. Adaptive Chafa covers initial loading and unsupported terminals only, and a Chafa failure no longer disables the native path.)
...
- `sudoku/`, `nonogram/`, `minesweeper/`, `solitaire/`, `le_word/`, `rubiks_cube/`, and `sliding_puzzle/` are daily puzzle games. Le Word has a single global daily word rather than personal runs. Rubik's Cube has one shared daily scramble and no personal mode. Sliding Puzzle pairs each deterministic UTC-daily difficulty with one saved random personal board; `sliding_puzzle/image.rs` owns its session-local placeholder catalog and bounded async render caches without extending the persisted game model. **Capable clients get one opaque native Kitty/iTerm2/Sixel image per board cell, not one raster per board.** `render_terminal_puzzle_tiles` encodes one image per tile value (index 0 is the gap) plus one per destination for the solved board, once per source/geometry/protocol; a move reorders those immutable cells, so input never waits on the encoder and only the two changed cells are retransmitted. The encoded set deliberately outlives the request that produced it — toggling the view off sets that request to `None` every time, and dropping the set there would re-encode the whole board on every `i` press. Unsupported clients get Chafa fragments cut from the solved-position preview, with the gap left blank. Both paths show subtle amber tile labels while unsolved; the solved cell set bakes a compact completion banner into the artwork and carries no labels, while loading or failure preserves the numbered fallback. The two halves share one per-session source-byte cache, so a resize re-renders without re-downloading, and a Chafa failure does not disable the native path: Chafa fits by aspect ratio and rejects non-square artwork, whereas the native encoder resizes exactly and does not care. Preview rendering is serialized and retains only the current request, native encoding stays one-at-a-time, `tick.rs` releases every session-held raster once the board stops being the open screen (nothing else can — `poll_image_tiles` is the only evictor and it stops running too), and persistent iTerm2/Sixel footprints are wiped before ratatui on disappearance, geometry/content changes, overlays, or protocol changes. **Finished work is shared process-wide:** `image.rs` keeps one `PROCESS_IMAGE_CACHE` of source bytes, Chafa previews, and native cell sets keyed by source/dimension/geometry/settings/protocol (a few hundred keys at most, each map dropped outright at `PROCESS_CACHE_CAP`), so one player's download and encode serves every session after it and a trip to the lobby costs nothing to come back from; the spawned tasks consult it before rendering and store into it after. Every spawned stage ends in `finish_stage`, the one place that logs a failure (`tracing::warn!` with the stage and request key) and records `metrics::record_sliding_puzzle_image(stage, outcome)`; a run of `failed` on the `preview` stage is the artwork CDN refusing us. On Kitty a move costs two placement commands and two placement deletes, no pixels (see the root CONTEXT.md terminal side-channel entry).
...
- Sliding Puzzle's image view uses the existing size-limited remote-image downloader or an operator-authored local `file://` source canonicalized beneath `artwork/sliding-puzzle`. Local paths are configuration only and are never supplied by SSH users; traversal, missing/non-files, oversized bytes, and unsupported image data fail to numbered fallback. **That directory is a dev affordance only** — no Dockerfile stage copies `artwork/` into a runtime image, so `file://` sources resolve only under the Compose bind mount. **Tile geometry is adaptive, not fixed:** `image_tile_geometry` takes the largest tile the board area allows, from 6×3 up to 16×8 cells (`MIN_`/`MAX_IMAGE_TILE_HEIGHT`, width always twice height), so the request key changes with the terminal size. Capable terminals get native per-cell rasters (see the Source Map entry above); everyone else gets `files::inline_image` Chafa fragments, with the current board permutation moving solved-position fragments and the gap left blank. `App::tick` polls completion and dirties the frame, while loading, invalid geometry, or source failure renders numbered fallback tiles in the same geometry. `App::content_area()` and `arcade::ui::SHOW_GAME_BOTTOM_BAR` are the single definitions of the board rect that the draw path, mouse hit-testing, and the pre-frame raster wipe all read — they have to agree cell for cell or placements land in the wrong rect, and `sliding_puzzle::ui::native_placement` is the one predicate deciding whether native tiles are on screen at all. Tests use temporary local fixtures and synthetic styled lines without network access.
...
- **Sliding Puzzle art is hotlinked from a third party.** `PLACEHOLDER_IMAGE_URLS` hardcodes three signed `fastly.picsum.photos` CDN URLs. Lorem Picsum is a free placeholder service with no SLA, no published terms, and no rate-limit policy; the `hmac` query params are generated by their signing key, so a key rotation on their side 403s all three at once and the image view degrades to numbered tiles for everyone with no server-side signal. Every other Arcade asset is embedded at build time via `include_bytes!`. Committing licensed artwork and embedding it the same way would remove the runtime dependency, the per-session download, and the `file://` code path together; it needs an artwork/licensing decision first.
