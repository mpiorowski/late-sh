...
- `ui.rs` renders the lobby and exposes Arcade-only bottom-bar/status helpers. The lobby carves `hub::dailies::ui::ARCADE_STRIP_HEIGHT` rows off the top for the quest strip (heading + two dailies + weekly, from `QuestState`) whenever the area is at least 18 rows tall; active games never show it.
