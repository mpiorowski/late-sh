...
- Last updated: 2026-08-21 (every daily puzzle rolls over at UTC midnight inside a live session via `workspace::refresh_daily_games`, instead of only on reconnect)
...
- `twenty_forty_eight/`, `tetris/`, and `snake/` are high-score games. Snake's board glyphs (food, power-up star, rocks, the snake, the frame) come from `theme::` tokens, not the ANSI 16: every cell is painted over `theme::BG_SELECTION()`, where a fixed color can land on a background of the same hue (a magenta star was invisible on Amoled Ultra Violet).
