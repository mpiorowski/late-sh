...
- Last updated: 2026-09-11 (Sliding Puzzle resolves immutable daily artwork assignments from Postgres; #puzzle-art contributions join the pool through staff approval, alongside the embedded starter illustrations.)
...
- Sliding Puzzle defaults to image mode for daily and personal boards. A manual `i` toggle saves `users.settings.sliding_puzzle_image_mode` through the existing FIFO save worker; session initialization reloads it after queued saves settle. The JSONB update preserves other settings. Loading or failed artwork automatically renders playable numbered tiles; failures show an unavailable hint and `i` twice retries. Automatic fallback and dev previews do not write this preference. Image tiles use a DB-assigned embedded or community source. `sliding_puzzle_artworks` contains reviewed entries; `sliding_puzzle_daily_artworks` fixes one image per date under a transaction advisory lock. Staff approval in #puzzle-art makes a submission eligible from the next UTC day; least-recently-featured selection excludes already-assigned adjacent images. Personal boards select only embedded originals by saved seed. The source UUID/index participates in all byte/native/Chafa cache keys, and hosted community images reuse the guarded downloader. See `late-ssh/assets/sliding-puzzle/README.md` for contributions and operations. **Tile geometry is adaptive, not fixed:** `image_tile_geometry` takes the largest tile the board area allows, from 6×3 up to 16×8 cells (`MIN_`/`MAX_IMAGE_TILE_HEIGHT`, width always twice height), so the request key changes with the terminal size. Capable terminals get native per-cell rasters (see the Source Map entry above); everyone else gets `files::inline_image` Chafa fragments, with the current board permutation moving solved-position fragments and the gap left blank. `App::tick` polls completion and dirties the frame, while loading, invalid geometry, or source failure renders numbered fallback tiles in the same geometry. `App::content_area()` and `arcade::ui::SHOW_GAME_BOTTOM_BAR` are the single definitions of the board rect that the draw path, mouse hit-testing, and the pre-frame raster wipe all read — they have to agree cell for cell or placements land in the wrong rect, and `sliding_puzzle::ui::native_placement` is the one predicate deciding whether native tiles are on screen at all. Tests use embedded assets and synthetic styled lines without network access.
...
- Development profiles additionally expose `a` (next art) in Sliding Puzzle. This cycles the approved, active pool including tomorrow’s approvals, switches to daily image view, and changes only the current session’s art. It never changes daily assignments, approval dates, or saved boards; production services reject this operation and omit the control.
- Sliding Puzzle approvals are additive; thumbs-down/removing thumbs-up do not revoke. Deleting a submission retires future selection, while existing daily assignments retain the approved copy.
