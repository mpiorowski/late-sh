...
- Last updated: 2026-09-07 (Share cards: every finished daily has a result card, `s` copies it to the clipboard, the lobby's `s` copies the day card, and the lobby lists the dailies first.)
...
- `share.rs` owns the share-card grammar: `ShareCard` (a header, at most ten body rows, the fixed `ssh late.sh` footer), the closed `Glyph` set with its emoji and ASCII spellings, `render`, the per-game epochs behind puzzle numbers (never move one: every pasted card would renumber), and the day card (one glyph per daily in lobby order plus the quest streak). Each daily has a pure `share.rs` beside its `state.rs` (`from_state` returns `None` until the daily is finished, `card` takes plain data and is whole-state tested in `share_test.rs`). Each `share.rs` also exposes `is_ready`, the cheap gate the key line reads and `from_state` checks before building anything. Copying, the banner, and the `share_cards_total` metric live in one place, `share_card` in `input.rs`; `s` is intercepted there only while a card exists, so a Le Word letter keeps working on an open board. Four cards need histories the saves never kept and now carry session-local: Sudoku's `box_finish_rank` (which third of the solve completed each box), Minesweeper's `click_log` (the strip of safe/flag/boom), Rubik's `move_log` (the face ribbon), Sliding Puzzle's `blank_visits` (the heatmap). They ride the board snapshots, so switching difficulty and back keeps them, and a board restored from a save starts empty: the card then shows its stat line with a neutral body (all green boxes, no strip, no ribbon, an all-white map). The Nonogram card is the finished picture in half-blocks, which is the solution on purpose: it is the one card that gives something away, and the hard 20x20 is what sets the ten-row cap.
