...
4. **chips**: `balance · this month`, the off-board note, then the newest `PROFILE_LEDGER_ROWS` ledger rows, one line each (`ledger.rs`): date, signed delta, a label per `ChipMove` (exhaustive), a readable detail when the ref means something to a person (gift counterparties and gild parties, the gild row's buyer and author, resolved to usernames by `ProfileService`; drinks, SKUs, links), and an `off` marker on rows `ChipMove::counts_as_earnings` ignores. Anyone can open anyone's profile, so this is where a place on Top Chips is audited by the room.
