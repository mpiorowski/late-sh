...
1. **Hero.** The bonsai as the neofetch logo on the left (34 columns, at most 16 rows; a taller tree loses crown rows, never the pot), the late.fetch grid on the right: the name and a rule, then `country`, `local`, `chips` (balance and this month's Top Chips figure, the board's own sum via `UserChips::earned_this_month`), `gilds` and `gallery` when non-zero, `created`, `ide`, `os`, `terminal`, `theme`, `langs`. Unset values are dim, never absent. Below 83 body columns the hero stacks: tree above grid. That is the only reflow.
...
4. **chips**: `balance · this month`, the off-board note, then the newest `PROFILE_LEDGER_ROWS` ledger rows, one line each (`ledger.rs`): date, signed delta, a label per `ChipMove` (exhaustive), a readable detail when the ref means something to a person (gift counterparties resolved to usernames by `ProfileService`, drinks, SKUs, links), and an `off` marker on rows `ChipMove::counts_as_earnings` ignores. Anyone can open anyone's profile, so this is where a place on Top Chips is audited by the room.
