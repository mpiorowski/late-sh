...
pub(crate) struct ModModalState {
...
pub(crate) struct ModLogLine {
    pub text: String,
    pub kind: ModLogKind,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum ModLogKind {
    Input,
    Separator,
    Info,
    Success,
...
impl ModModalState {
    ...
    pub(crate) fn command_input(&self) -> &TextArea<'static> {
        &self.command_input
    }

    pub(crate) fn log(&self) -> &VecDeque<ModLogLine> {
        &self.log
    }
    ...
    pub(crate) fn autocomplete_matches(&self) -> &[MentionMatch] {
        &self.mention_ac.matches
    }

    pub(crate) fn autocomplete_selected(&self) -> usize {
        self.mention_ac.selected
    }

    pub(crate) fn autocomplete_query(&self) -> Option<(usize, char, String)> {
    ...
    pub(crate) fn update_autocomplete_matches(
        &mut self,
    ...
    pub(crate) fn ac_move_selection(&mut self, delta: isize) {
    ...
    pub(crate) fn ac_confirm(&mut self) {
        if !self.mention_ac.active || self.mention_ac.matches.is_empty() {
            return;
        }
    ...
    pub(crate) fn ac_dismiss(&mut self) {
        self.mention_ac = MentionAutocomplete::default();
    }

    pub(crate) fn append_input(&mut self, command: &str) {
    ...
    pub(crate) fn append_info(&mut self, line: impl Into<String>) {
        self.push_log(line.into(), ModLogKind::Info);
    }

    pub(crate) fn append_error(&mut self, line: impl Into<String>) {
    ...
    pub(crate) fn scroll_log(&mut self, delta: i16) {
        if delta < 0 {
