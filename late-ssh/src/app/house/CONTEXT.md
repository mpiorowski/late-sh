...
- **Activity.** Poker/asterion/tron publish `RoomGameEvent::SeatJoined` onto the registry's shared channel; blackjack's `BlackjackEvent::SeatJoined` is translated by a forwarder. `start_seat_activity_task` turns those into `ActivityEvent::sat_down` — the house equivalent of the rooms registry choke point. `game_won` publishing and chip payouts are unchanged inside each runtime (asterion daily escape, tron cooldown payouts, poker/blackjack settlements).
...
- **Screen::HouseTable**: outside the Tab cycle, entered only from the modal (or backtick). Joined every chat-surface gate in `app/input.rs` (`is_chat_composer_context`, overlay handling/close, reaction-leader Esc, scroll routing, `chat_click_room_id`, composer/scroll click hit tests). Esc peels: selected table-chat message → close to modal. `compose_room_switch_allowed` stays Dashboard-only.
