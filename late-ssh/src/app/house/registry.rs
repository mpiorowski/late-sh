...
//! per `HouseTable` variant. Mirrors the rooms-era table managers minus the
//! `game_rooms` persistence — house tables have no DB row, so the services
//! run with `rooms_service: None` and live for the whole process (Asterion
//! keeps its stop-when-empty behavior and is recreated on the next enter).
...
impl HouseTableRegistry {
    ...
    pub fn enter(
        ...
        match table {
            HouseTable::Poker => Some(HouseTableClient::Poker(
                crate::app::house::poker::state::State::new(
                    self.poker_service(),
                    user_id,
                    chip_balance,
                ),
            ...
            HouseTable::Blackjack => Some(HouseTableClient::Blackjack(
                crate::app::house::blackjack::state::State::new(
            ...
            HouseTable::Asterion => {
                ...
                Some(HouseTableClient::Asterion(
                    crate::app::house::asterion::state::State::new(svc, user_id, session_id),
                ))
            }
            HouseTable::Tron => Some(HouseTableClient::Tron(
                crate::app::house::tron::state::State::new(self.tron_service(), user_id),
            )),
        }
    }
        ...
        match table {
            HouseTable::Poker => {
                ...
                HouseOccupancy {
                    ...
                    in_round: !matches!(
                        snapshot.phase,
                        crate::app::house::poker::svc::PokerPhase::Waiting
                            | crate::app::house::poker::svc::PokerPhase::Showdown
                    ),
                }
            }
            HouseTable::Blackjack => {
                ...
                HouseOccupancy {
                    ...
                    in_round: matches!(
                        snapshot.phase,
                        crate::app::house::blackjack::state::Phase::BetPending
                            | crate::app::house::blackjack::state::Phase::PlayerTurn
                            | crate::app::house::blackjack::state::Phase::DealerTurn
                    ),
                }
            }
            HouseTable::Asterion => {
            ...
            HouseTable::Tron => {
                ...
                HouseOccupancy {
                    ...
                    capacity,
                    in_round: snapshot.phase == crate::app::house::tron::state::TronPhase::Running,
                }
            }
        }
    }
    ...
    fn blackjack_service(&self) -> BlackjackService {
        ...
        slot.get_or_insert_with(|| {
            ...
            self.forward_blackjack_seat_joins(event_tx.subscribe());
