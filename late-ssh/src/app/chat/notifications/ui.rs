...
pub fn draw_notification_list(frame: &mut Frame, area: Rect, view: &NotificationListView<'_>) {
    ...
    for (row, item_area) in layout.iter().copied().enumerate() {
        ...
        // Either cursor clears the dot: the feed watermark frozen on entry
        // (so the dots survive the visit that read them) or the read cursor of
        // the room the mention was said in.
        let is_unread = view
            .marker_read_at
            .is_none_or(|last_read_at| item.created > last_read_at)
            && item
                .room_read_at
                .is_none_or(|room_read_at| item.created > room_read_at);
