...
Hub Shop room effects add render-time top sections in the cozy room rail. Active `room_bump` rooms render first under a dedicated `bumped` section; other highlighted/effect rooms render under `boosted` above Favorites. `room_highlight` still flows through `ChatRoomListView.highlighted_room_ids`; richer active effects flow through `ChatRoomListView.active_room_effects`. Hit testing uses the same visual slot list, so boosted/bumped room clicks stay aligned with rendering. Avoid duplicating promoted rooms in their original Favorites/Core/Channels/DM sections. Active room effects also render a one-line strip above the selected room's messages: Room Spark animates a short spark label with seconds remaining, Room Glow marks the room as glowing, Pinned Vibe shows the curated vibe payload, and Room Bump marks the room as bumped.
