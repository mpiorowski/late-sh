...
Room tails deliberately carry **no** read cursor. Render inserts one synthetic `new messages` divider before the first message from someone else past this terminal's **AFK line** (`ChatState::afk_lines`), never past `chat_room_members.last_read_at`; see `The AFK line` below for why. The divider is render-only state in the chat row cache; do not persist it or count it as a chat message.

### The AFK Line

One horizontal rule, one question: **when did the person at this terminal stop attending this room?** The `new messages` divider draws against it, `/history` opens there, and a bare `/summary` reads from it. There is deliberately no second cursor (no account-wide "what you have been told" watermark, no read marker): every past attempt to derive one of those produced the bugs listed below.

**It is not a read cursor, because reading cannot be observed.** Reading a room produces no input at all, so no signal the server can see distinguishes a person reading in silence from a person who walked away. The line is therefore about something the reader *did* (went quiet, left, spoke), not about what they took in.

**Placement** (`ChatState::afk_lines`, `sync_afk_line`, `seed_afk_lines_from_device`, `clear_afk_line`). Two sources, one map:

- **Within a session**: `App::tick` mirrors `last_input_at.elapsed()` into chat each tick, the same way `viewer_tz` is mirrored, because chat is what knows which room is on screen. Past `AFK_LINE_IDLE` (10 min) the visible room gets a line stamped at `now - idle`, the moment the keyboard went quiet rather than the moment we noticed.
- **Across sessions, per device**: `Drop for App` writes `user_ssh_keys.left_at` (migration 166, `UserSshKey::set_left_at`, through `ProfileService::set_key_left_at`) stamped at `now - last_input_at.elapsed()`, again the moment the keyboard went quiet, not the moment the connection closed: a terminal parked open overnight and shut in the morning left last night. `session_bootstrap::load_device_state` **takes** it (`UserSshKey::take_left_at`, an `UPDATE ... RETURNING` that nulls the column), `SessionConfig::key_left_at` carries it into `ChatSession::device_left_at`, and the first room snapshot seeds every room's line from it, once. Taking rather than reading is what makes a lost write safe: the drop write is fire-and-forget, and a session whose write never landed (process killed mid-drain, DB blip, key re-pointed by account linking) must not hand the next session a leave from *before* itself, which would draw a divider and a `/summary` window over messages already read. A lost write degrades to "no line, 24h default", never to a wrong line. Leaving the app leaves every room at once, so one instant applies to all of them; a room joined later in the session was not one you left. Keyless sessions (ghost bots, tests) have no device to remember it on and start with no line.
- **Each line knows its source** (`AfkLine { at, source: Idle | DeviceLeave }`). The divider and `/history` use only `at`; `/summary` hands the source over as a separate `SummaryWindow` arm because the overlay head names which line it read, and "since you went quiet" is a claim about this session while "since your last session on this device" is a claim about the previous one.

One sentence governs the line: **it marks what you missed, from when you went quiet until you speak again.** Everything else follows from that sentence, and anything the sentence does not predict is a bug.

- **Only the visible room collects one in-session.** A room you are not looking at was never being attended, and its rail badge already says what is waiting.
- **A room that has a line keeps the one it has.** The line says when you left; staying away longer does not move when you left, and a device seed never overwrites a line the session already placed.
...
- **`/history` and `/summary` read the line and never spend it.** Opening scrollback is how you go and look at the backlog, not proof you got through it; a summary says what is below the line, it does not move where the line is. Asking `/summary` twice therefore reads the same stretch twice (plus whatever landed since); the cooldown bounds that, and the reply that usually follows a catch-up is what moves the line.
- **Per device, never per account.** "Did the person at this terminal step away" is a fact about this terminal; your phone being idle says nothing about your desktop. The cost is that a room you read on the phone all evening is summarized again on the desktop next morning. That is what "since I was last here" means, and it is accepted over an account-wide cursor that could not tell reading from presence. One key on two machines reads as one device; last write wins.
...
A fourth arrangement, tried and removed in the same pass: a per-account `/summary` watermark (`chat_summary_reads`, advanced on delivery). It answered "what have you been told" correctly but was a second line with its own rules, and the per-device mark answers the question people actually ask (what happened since I was last here) with the one line that already exists.
...
- `last_read_at` drives unread counts **and nothing else**. It is a presence cursor (`mark_room_read` fires whenever a message lands in a visible room); it is not what the `new messages` divider or the `/summary` window rest on. See `The AFK Line`.
...
- `/summary` asks the AI for a catch-up of the visible public room, from the room's AFK line (when you went quiet, or when you last left the app on this device; 24h when there is no line), or exactly the window you type (`/summary 6h`, `/summary 90m`, up to 48h); see §14 Summary. `/history` opens the scroll-back modal, at the first message you missed when the room has an AFK line; see §14 History Modal.
...
Scroll-driven room history (`history_modal/`), opened by `/history` or by a search hit/mention older than the loaded tail. Three open modes on `ChatHistoryModalState`: `open_at_tail`, `open_at_message` (anchored, highlighted), and `open_at_unread`. `/history` picks between tail and unread by the room's AFK line (`ChatState::afk_lines`, in-session or seeded from the device mark); the cutoff is always that, never `chat_room_members.last_read_at`, which records presence rather than reading. `/history` only *reads* the line and never clears it: opening scrollback is how you go and look at the backlog, not proof you got through it. `ChatService::load_history_unread_task` resolves the exact first unread server-side (`ChatMessage::first_unread_after`, same read-scope/ignore/system filters as history pages, so it can sit further back than the tail's 500) and answers through the anchor pipeline; when nothing qualifies it degrades to a plain tail page under the same request id, which the modal accepts as a tail open. The modal draws the same `new messages` divider as the live tail, before the first loaded message from someone else past the cutoff (`unread_divider_target`); a room with no AFK line gets no divider, mirroring the live tail.

### Summary

`/summary` is the AI room catch-up: one call summarizing the visible public room, either since the room's AFK line or over a window they typed (`/summary 6h`). `app/ai/summary.rs` owns the whole pipeline (`SummaryService`, translate.rs-shaped); `ChatState` only parses the command, hands over the line (`parse_summary_arg` + `afk_lines`), and drains `SummaryEvent`s in tick: ready summaries open the shared `Overlay`, waiting in `pending_summary_overlay` when another overlay is already up (a banner says so, and the summary takes the surface as soon as it frees); everything else banners.

- **Window**: the command layer parses the argument into a closed `SummaryArg` (`CatchUp`, `Window`, `Unparseable`, `TooShort`, `TooLong`, each with its own banner) and resolves `CatchUp` against the room's `AfkLine` into one of four `SummaryWindow` arms. The line is a fact about this terminal that lives only in the session (and on the device key), so the session is what supplies it, and the service keeps no cursor of its own. `summary::window_start` resolves every arm and reports a closed `SummaryBasis` plus a `capped` flag alongside the start, so the overlay head can say which sentence it is telling. Every bare arm names its window in the head because the states are easy to confuse from the outside: posting clears the line, so the steady state for anyone who talks in a room is *no* line, and the head must not then claim they never stepped away.
  - `SinceWentQuiet(at)` (line placed by this session's idle) → exactly that instant; basis `WentQuiet`, head "since you went quiet · stamp". **Nothing widens it.** A line ten minutes old means a ten-minute catch-up, which is the entire point of resting on when the keyboard went quiet.
  - `SinceLeftDevice(at)` (line seeded from the device mark) → the same window; basis `LeftDevice`, head "since your last session on this device · stamp". Separate arm because it is a claim about the previous session, not this one, and it applies to rooms never opened here.
  - `Default` (no line) → `SUMMARY_DEFAULT_WINDOW_HOURS` (24h), basis `Default`, head "since stamp · the last 24h", with no claim about absence either way. The only window that is handed out rather than derived.
  - `Explicit(duration)` → what the user typed, bounded only by the 48h max; the line is ignored outright.
  - Either line arm older than `SUMMARY_MAX_WINDOW_HOURS` is clamped to it and the head appends "· capped at 48h": the stamp is then the cap, not the moment they left, and the head says which.
...
- **Nothing is written on delivery.** The line is the reader's mark, not the summary's; a second `/summary` reads the same stretch again (see `The AFK Line`). A failed summary therefore costs nothing but the request.
...
- **`Empty` carries its basis**, because "nothing new since you went quiet" and "nothing said in the last 24h" are different claims, and the banner tells them apart.
