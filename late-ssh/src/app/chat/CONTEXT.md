...
- Inline image rendering detects likely image URLs in visible room messages, fetches them through the same secure downloader, rejects oversized decoded dimensions, retries transient failures with backoff, and caches an `InlineImagePreview` by message id. Each preview carries the RGB block fallback plus a bounded PNG raster preview. The scrolling chat rows always render the RGB block fallback; terminal-native images are lazy and only emitted inside the explicit image modal opened from a selected image message. Inline previews are best-effort; failures are intentionally silent/noisy only at trace level.
