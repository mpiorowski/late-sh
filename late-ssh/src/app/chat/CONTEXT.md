...
- oEmbed carries neither an image nor a sensitivity flag, so `fetch_tweet_media` gets both from **fxtwitter** (`api.fxtwitter.com/i/status/<id>`), the one third-party dependency in the pipeline. It is best effort and isolated in that single function: a failure costs the card its thumbnail (falling back to `procedural_ascii_art`), never the share. It sends an explicit `User-Agent` because fxtwitter answers `401` without one and `reqwest` sends none by default. **Known limitation:** the sensitive-post rejection rides on fxtwitter's `possibly_sensitive`, so while fxtwitter is unreachable an X share carries no NSFW gate.
