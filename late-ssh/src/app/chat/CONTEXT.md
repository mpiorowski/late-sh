...
Room tails deliberately carry **no** read cursor. Render inserts one synthetic `new messages` divider before the first message from someone else past this session's **AFK line** (`ChatState::afk_lines`), never past `chat_room_members.last_read_at`; see `The two lines` below for why. The divider is render-only state in the chat row cache; do not persist it or count it as a chat message.

### The Two Lines

Two horizontal rules can appear in a transcript, and they answer different questions. Keeping them apart is the whole design; collapsing them back into one cursor is what produced the bugs below.

| | `new messages` (AFK line) | summary watermark |
|---|---|---|
| Means | you were not at this terminal | what you have been told about this room |
| Scope | one SSH session, in memory | the account, in `chat_summary_reads` |
| Set by | `AFK_LINE_IDLE` (10 min) of keyboard silence | a delivered `/summary` |
| Cleared by | your own message landing, or `/summary` | never; it only carries forward |

**Neither is a read cursor, because reading cannot be observed.** Reading a room produces no input at all, so no signal the server can see distinguishes a person reading in silence from a person who walked away. Every cursor here is therefore about something the reader *did*, not about what they took in.
...
- **Returning does not clear it.** Clearing on the keystroke that brings you back destroys the line at the exact moment it is wanted. The clear is `push_message` seeing a message you authored (one place to look, and the authoritative one: several submit paths send, `/me`, `/roll`, `/cup`, a plain line, and they all land there), plus `/summary`.
...
- **Never leaves the process.** "Did the person at this terminal step away" is a fact about this terminal; your phone being idle says nothing about your desktop.

**The summary watermark** (`late-core/src/models/chat_summary_read.rs`, migration 165). Per (reader, room). See §14 Summary.
...
- `last_read_at` drives unread counts **and nothing else**. It is a presence cursor (`mark_room_read` fires whenever a message lands in a visible room); it is not what the `new messages` divider or the `/summary` window rest on. See `The Two Lines`.
...
- `/summary` asks the AI for a catch-up of the visible public room, from the end of your last catch-up of it (24h on the first one), or exactly the window you type (`/summary 6h`, `/summary 90m`, up to 48h); see §14 Summary. `/history` opens the scroll-back modal, at the first message you missed when this session has an AFK line; see §14 History Modal.
...
- **Window**: the command layer parses the argument into a closed `SummaryArg` (`CatchUp`, `Window`, `Unparseable`, `TooShort`, `TooLong`, each with its own banner) and hands the service a `SummaryWindow`. `SinceLastSummary` carries no instant at all: where the last catch-up stopped is a per-account fact, the service reads it itself (`ChatSummaryRead::summarized_through`), and a session is never given the chance to supply it. `summary::window_start` resolves both arms and reports a closed `SummaryBasis` alongside the start, so the overlay head can say which sentence it is telling.
  - `SinceLastSummary` with a watermark → exactly that instant, clamped only at the far end by `SUMMARY_MAX_WINDOW_HOURS`. **Nothing widens it.** A watermark ten minutes old means a ten-minute catch-up, which is the entire point of resting on what the reader was told.
  - `SinceLastSummary` with none → `SUMMARY_FIRST_RUN_WINDOW_HOURS` (24h), basis `FirstCatchUp`, and the head says "first catch-up here". This fires exactly once per reader per room.
  - `Explicit(duration)` → what the user typed, bounded only by the 48h max. A stored watermark is ignored outright.
- **The old 24h floor is gone, and must not come back as a blanket clamp.** `SUMMARY_DEFAULT_WINDOW_HOURS` widened *every* bare catch-up because the cursor underneath it was `chat_room_members.last_read_at`, which records presence rather than reading: a terminal parked on a visible room marks each arriving message read, so a marker-only window answered "nothing new" to exactly the person who missed the day. The fix was to stop deriving the window from a cursor that cannot know, not to pad it. The 24h number survives only as the first-run window, where it applies once and is named on screen.
- **The watermark advances on delivery**, to the `created` of the newest message the transcript actually held, never to `now()`. A message landing while the model call is in flight is newer than that and belongs to the next window rather than to a hole between the two; the char budget only ever drops the *oldest* end, so the stamped message is one the reader is about to read. Consecutive windows therefore abut exactly: every message is covered by at most one summary. `ChatSummaryRead::advance` is `GREATEST`-guarded so a narrow explicit window can never rewind the cursor and re-serve the same bullets.
- **A failed watermark write does not fail the summary.** It is logged and counted (`record_chat_summary_watermark_failed`) and the catch-up is still delivered: the cost is one overlapping next window, which is self-correcting, and refusing a paid-for summary over bookkeeping is the worse trade.
- **`/summary` clears the room's AFK line** at request time, not at delivery: asking for the backlog is the reader's act, and a summary that fails is still a room they came back to. That is the one place the two lines touch.
...
- **`Empty` is now a true statement.** "Nothing new since your last catch-up" is what it means, rather than the artifact of a presence cursor it used to be.
