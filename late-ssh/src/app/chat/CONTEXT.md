...
- **Across sessions, per device**: `Drop for App` writes `user_ssh_keys.left_at` (migration 166, `UserSshKey::set_left_at`, through `ProfileService::set_key_left_at`) stamped at `now - last_input_at.elapsed()`, again the moment the keyboard went quiet, not the moment the connection closed: a terminal parked open overnight and shut in the morning left last night. `session_bootstrap::load_device_state` reads it back with the rail layout, `SessionConfig::key_left_at` carries it into `ChatSession::device_left_at`, and the first room snapshot seeds every room's line from it, once. Leaving the app leaves every room at once, so one instant applies to all of them; a room joined later in the session was not one you left. Keyless sessions (ghost bots, tests) have no device to remember it on and start with no line.
...
- **Window**: the command layer parses the argument into a closed `SummaryArg` (`CatchUp`, `Window`, `Unparseable`, `TooShort`, `TooLong`, each with its own banner) and hands the service a `SummaryWindow`. `CatchUp { away_since }` carries the room's AFK line: it is a fact about this terminal that lives only in the session (and on the device key), so the session is what supplies it, and the service keeps no cursor of its own. `summary::window_start` resolves both arms and reports a closed `SummaryBasis` alongside the start, so the overlay head can say which sentence it is telling.
  - `CatchUp` with a line → exactly that instant, clamped only at the far end by `SUMMARY_MAX_WINDOW_HOURS`; basis `AwaySince`, head "since you stepped away". **Nothing widens it.** A line ten minutes old means a ten-minute catch-up, which is the entire point of resting on when the keyboard went quiet.
  - `CatchUp` with none → `SUMMARY_DEFAULT_WINDOW_HOURS` (24h), basis `NoAwayMark`, and the head says so ("the last 24h, you never stepped away here"). This is the only window that is handed out rather than derived, and it is named on screen.
...
- **`Empty` carries its basis**, because "nothing new since you stepped away" and "nothing said in the last 24h" are different claims, and the banner tells them apart.
