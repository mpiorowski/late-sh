...
- Last updated: 2026-08-08 (translation goes two-way: English targets now translate Latin-script languages via a cached model `same_language` verdict instead of a script shortcut, replies translate only the reply text (the `> @author:` quote line is stripped everywhere translation looks), and an opt-in "Translate my messages to English" setting pre-warms the shared cache at send/edit time; see §14 Translation)
- Status: Active
...
Translations:
- `message_translations` is the shared translation cache, primary key `(message_id, target_lang)`, `ON DELETE CASCADE` from the message. Migration 136; migration 137 adds `same_language` (a cached "already in this language" verdict whose `body` keeps the judged text, rendered as nothing).
...
- **Authors can pre-share in English.** `Ctrl+O` → Settings → Translation → "Translate my messages to English" (`users.settings.translate_mine_to_en`, off by default): the send and edit paths (`ChatService::pretranslate_for_author`) fire an `En` request for the author's own message, warming the shared cache so English readers see it without asking. One call per message written by the opted-in author, spent from the same daily cap.
...
- Target language, auto mode, and the author-side English share are per account (`users.settings`: `translate_to`, `auto_translate`, `translate_mine_to_en`), edited under `Ctrl+O` → Settings → Translation; the first two sync into `ChatState` each tick, the third is read by `ChatService` at send/edit time.
