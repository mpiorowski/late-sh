...
- Last updated: 2026-09-11 (#puzzle-art is a permanent Core submission room: validated/rehosted images become pending pool entries; staff thumbs-up approves in the reaction transaction. Replies remain discussion, submissions are immutable, and deleting one retires future puzzle selection.)
- Status: Active
...
- `/public #room` (alias `/join #room`) opens a public room for the caller, creating it as opt-in (`auto_join=false`) if needed. Opening an existing room preserves its auto-join and permanent flags.
...
Puzzle artwork submissions:
- `#puzzle-art` is a permanent, auto-joined Core room seeded by migration 182, with topic and `/rules`. Top-level sends accept exactly one public image URL plus optional title (120 chars); existing pasted/uploaded image URLs follow the same path. The gate uses the public topic room’s DB slug, kind, and visibility, covering IRC and staff sends without affecting private rooms with the same name. Replies remain normal discussion and never create pool entries.
- `puzzle_art.rs` parses submissions, uses the guarded downloader, validates square PNG/JPEG/WebP (256–4096 per side, <=10 MiB), decodes with allocation limits, and produces a static PNG. `files::image_upload::upload_puzzle_artwork` stores it at a content-addressed S3/R2 key; the review message points to this managed copy. Missing upload config and invalid images return composer errors. Submission messages bypass drunk text so review identity stays exact.
- The message and `sliding_puzzle_artworks` pending entry publish in one transaction, with source URL, image URL/hash, title and snapshot username credit. Active duplicate content is refused. A DB trigger on `chat_message_reactions` approves only admin/moderator 👍 (including skin tones), atomically and once; eligible from next UTC day. Other users, thumbs-down, removal, and replacement do not revoke. The approving session gets a private receipt.
- Submission bodies cannot be edited (service error plus DB guard); deleting one retires its pool entry. Existing daily assignments keep their approved copy. See `late-ssh/assets/sliding-puzzle/README.md` and Arcade context for daily selection/rendering. No mutable process-global approval state or runtime redeployment is involved.

Image uploads and inline rendering:
- File-upload storage is optional per profile: `Config.files` is `Some(FilesConfig)` in prod (endpoint/bucket/URL literals plus `LATE_FILES_S3_ACCESS_KEY_ID`/`LATE_FILES_S3_SECRET_ACCESS_KEY` env secrets). Dev defaults to local disk (`tmp/uploads`, ignored by git), served at `http://localhost:4001/api/dev-files/...`; no credentials are needed. Explicitly setting both R2 credentials in `.env.local` retains the prod-bucket opt-in; a half-set pair is a startup error. At startup, `files::local::configure` registers this local origin only in dev. Generated object URLs resolve directly to bounded local reads for submission validation and rendering, with strict generated-key validation; other URLs retain all SSRF checks. The file-serving endpoint returns 404 in prod. Dev `force_admin` also grants the connecting SSH user the database admin flag so moderation triggers agree with the UI.
...
1. `/public #room` gets or creates a public topic room and joins the caller. New rooms default to `auto_join=false`; existing rooms retain their auto-join setting, so `/join #puzzle-art` cannot remove Core auto-membership for future users. `/join #room` is an alias for it, parsed in the same `parse_public_room_command` so the two can never drift; it exists because IRC users type `/join` first. Public rooms are hosted, not owned: opening one grants nothing, and a brand-new one posts two plain system-bot lines (deliberately without the system-line prefix, so they render as messages), one in the room asking the creator to describe it and one in #moderators reporting it.
