...
- **The hiccup belongs to the top of the ladder.** A `*hic*` is dropped into an existing gap at 60% of messages for wasted, rolled twice, so 84% of its messages carry one and 35% carry two: one message is enough to read as wasted. Sloshed rolls once at 8%, a rare hint of what is coming; tipsy and buzzed never hiccup at all, so the stammer stays the marker of the top level rather than of drinking. `only_the_top_of_the_ladder_hiccups` pins the bands.
