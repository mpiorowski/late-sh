...
- The cap check and the credit are two statements, not one locked transaction (the insert and the credit already were): two shares by one person landing in the same instant can both pass the count and pay a fourth. Each share is seconds of extraction, so the window is theoretical; noted, not guarded.
