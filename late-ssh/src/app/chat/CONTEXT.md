...
- `/status` is the one presence command: `focus 🍅`, `working 💻`, `building 🛠️`, `reading 📖`, `vibing 🎶`, `gaming 👾`, `eating 🍽️`, `away 💤`. The set is closed (`common/status.rs`), and its glyphs must stay disjoint from every purchasable badge; `status_test.rs` scans the seed migrations and fails a collision. Free text was deliberately removed: a status costs nothing, where a rented title costs chips and passes an AI screen, so it cannot be a place to write anything at all.
  - **The one rule, and it governs everything:** `/status <word> <minutes>` counts down, survives you posting, and retires itself at zero. `/status <word>` with no minutes is open-ended, never expires, and is cleared by your next chat message. `/brb` is exactly `/status away` with no minutes, which is how "back in a minute" works without a second mechanism. `/status off` clears; bare `/status` opens the picker. Because the rule is invisible in the badge, every banner and the picker's live hint spell it out, both through `chat/input.rs::status_set_message` so the two cannot drift.
