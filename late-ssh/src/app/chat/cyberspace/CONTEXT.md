...
**Our own `/cs` commands work from a room composer, and never reach their API.** `handle_room_composer_input` parses the draft with `parse_cyberspace_command` before submitting: `/cs chat`, `/cs mail` and `/cs mail @user` are answered locally (the picker opens *over* the room, the conversation starts, and the user keeps their place), every other `/cs …` is refused with a banner naming the three, and anything the parser does not recognise is sent exactly as typed, because `/me`, `/dice`, `/mute` and friends are theirs and expand server-side. Commands that would move the user (`/cs`, `/cs post`, `/cs link`, `/cs unlink`) stay with the main chat composer, where moving them is not a surprise. The feed pane advertises no commands at all: it has no composer to type them into.
