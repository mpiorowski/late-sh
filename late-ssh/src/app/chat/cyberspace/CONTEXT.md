...
- Guilds: same mechanism as cIRC, no surface here yet (section 9).
...
- Keys (linked): `j/k` move, `g` top of the current view, Enter opens the selected thread (or whatever the notification names, section 6b), `r` refreshes the feed / opens reply in a thread / reloads notifications, `p` compose, `c` copies the entry's public link (`{WEB_URL}/{username}/{slug}`, through `app.pending_clipboard` like every other copy in the app; entries with no slug have no link and say so, and the notifications view has no local entry to link at all), `n` moves the rail to the `notifications` row, `b` (or Esc via the shell's escape chain, `escape_to_root`) drops an open thread back to the row's own view; `b` with nothing open steps off the `notifications` row back to `feeds`. Unlinked, the pane is the pitch funnel: Enter opens the link modal, everything else falls through so global keys keep working.
...
- **Each rail row carries its own badge, and they never sum.** `feeds` shows `unread_entries()`, entries published since this user last read the feed, counted locally out of the probe page (below); `notifications` shows `unread_notifications()`, their own counter endpoint. Opening a row clears only its own half.
...
- No guild surface; cIRC's and C-Mail's own gaps are listed at the end of section 9.
...
**Shape.** Linking cyberspace gives the rail its own collapsible `cyberspace` section (`RoomSection::Cyberspace`, shortcut `y`) holding **`feeds`** (`RoomSlot::Cyberspace`, the feed/thread pane, moved out of Core and relabelled, because the chat rooms beside it are cyberspace too), **`notifications`** (`RoomSlot::CyberspaceNotifications`, the same pane with its root view on the notifications list), one row per added chat room, and one per pinned C-Mail conversation (`RoomSlot::CyberspaceMail`). The section renders only while `cyberspace_linked`, so an unlinked user's rail is untouched.

**Our own `/cs` commands work from a room composer, and never reach their API.** `handle_room_composer_input` parses the draft with `parse_cyberspace_command` before submitting: `/cs chat` and `/cs mail` are answered locally (their pickers open *over* the room), `/cs mail @user` starts the conversation and deliberately walks the user into it (below), every other `/cs …` is refused with a banner naming the three, and anything the parser does not recognise is sent exactly as typed, because `/me`, `/dice`, `/mute` and friends are theirs and expand server-side. Commands that would move the user (`/cs`, `/cs post`, `/cs link`, `/cs unlink`) stay with the main chat composer, where moving them is not a surprise. The feed pane advertises no commands at all: it has no composer to type them into.

**Leaving a room goes back to the row it was entered from** (`ChatState::cyberspace_return_row`, a `CyberspaceRow` noted before the room takes the selection). Landing on `feeds` after arriving from the notifications row, which is what a mention jump does, reads as being thrown somewhere else.
...
**A room writes through the chat composer slot, not its own box.** `draw_room` renders only the header and the conversation; the composer `TextArea` lives on `OpenRoom` but is drawn by `chat::ui` into the footer block it shares with the pane's hint line (titled with the room's name while one is open). Drawing a second bordered input inside the pane stacked two boxes over each other and made a room look unlike every other room. It is single-line for the same reason: that slot is one row, and their API takes one message per send. **Entering a room lands in reading mode, not in the composer** (`OpenRoom::composing`): `i` or Enter focuses it, Esc drops back out of it, and a second Esc leaves the room. `app::input` routes every keystroke into the composer only while it holds focus, so a room being read keeps `j/k`/`g`, the rail shortcuts, and every other global letter, exactly like every other room in the rail. Focusing on entry made all of them text. Sending follows the profile's `keep_composer_focused` tweak, the same as the main chat composer.

**Writing to someone new is `/cs mail @user`** (alias `/cs dm @user`), which is their documented way to start a conversation: `POST /v1/cmail { recipientUsername }` is idempotent, returning the existing conversation (`200`) or creating one (`201`). `CmailStarted` pins the row, banners it, and records the id in `started_cmail`; the chat tick takes it, selects the row and pulls the user to Home, because naming someone is asking to write to them, not asking for a new rail row to go find. Their caps: 5/min, 30/hour, 50/day on starting conversations, 15/min, 150/hour, 300/day on messages.
