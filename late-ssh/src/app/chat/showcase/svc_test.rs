...
async fn unread_count_uses_showcase_read_cursor() {
    ...
    // The cursor and the new row can land on the same microsecond; push the
    // row strictly past the cursor so `created > last_read_at` is decisive.
    late_core::test_utils::bump_created_past_now(
        &client,
        "showcases",
        "user_id = $1 AND title = 'Later Project'",
        &[&author.id],
    )
    .await;
