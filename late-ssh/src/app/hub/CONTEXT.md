...
- `shop_consumable_effects` stores active user/room effects. Room-targeted Chat consumables activate against the currently selected Home chat room and are rejected before purchase when no room is selected. Active room effects are projected into Shop snapshots as `active_room_effects`; Home chat renders a one-line active-effect strip above the selected room's messages, promotes active `room_bump` rooms into a dedicated top `bumped` rail section, promotes other active-effect rooms above Favorites in the `boosted` section, and adds rail suffix/color hints for spark/glow/vibe/bump states. `room_highlight` remains projected as `highlighted_room_ids` for existing boosted-room behavior. Bot Username Color is projected as `bot_username_color_active` and brightens bot/graybeard/dealer author labels for the buyer while active. Message Accent is projected through active user effect kinds and brightens the buyer's own message bodies while active.
...
- Add deeper behavioral hooks for Chat consumables after the first visible pass, especially one-shot "next message" consumption for Message Accent and real ordering semantics for Room Bump.
