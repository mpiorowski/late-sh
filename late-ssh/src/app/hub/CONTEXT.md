...
- Last updated: 2026-08-25 (custom titles: migration 150 adds a 2,000/24h and a 60,000/30d `title_custom_*` SKU whose text the buyer types into a prompt, screened by `app/ai/screen.rs` before the purchase transaction opens, so a refused title costs nothing and an unscreened one never ships. Previous: badges, flags, and titles became rentals: migrations 148/149 seed a 24h and a 30-day tier per badge/flag and 36 curated titles, retire the permanent badge SKUs (`active = false`), and the Shop now sells four rental kinds through one shared shape in `late-core/src/models/rental.rs`)
