...
- The `dynamic_bonsai` feature unlock (slot `bonsai_variant`) was retired by migration 177 when the living bonsai became everyone's only tree: `active = false`, every equip cleared, purchase history kept. Nothing on sale equips a slot any more, so `ShopState` has no equip flow and `ShopService` no equip wrappers.
