...
## Metadata
- Scope: `late-ssh/src/app/lobby` — the single front door for multiplayer play: the `Ctrl+G` modal, the backtick workspace cycle, and the two game domains it fronts (`daily/` async correspondence matches, `house/` live fixed tables).
- Last updated: 2026-08-15 (Lateania joins the backtick cycle's door leg via a 5-minute recency window instead of a detached session; see the `workspace.rs` row)
...
| `workspace.rs` | The backtick cycle (`GameWorkspace`, `cycle_game_workspace`, pure `next_workspace` + its unit tests). Consumes `arcade/workspace.rs` for the Arcade-daily leg; hopping out of an Arcade stop clears `is_playing_game` (boards save move-by-move, nothing else to close). The final leg is `GameWorkspace::Door`, the door games in hub sidebar order, with two liveness models. Lateania first: it has no detached session (`set_screen` away tears the world session down with an autosave), so its stop is the recency window `App::lateania_detached_at` — armed only by the in-world backtick detach (`lateania/input.rs` returns `InputAction::Detach`, mapped in `lateania/screen.rs`), cleared by an explicit Esc-Esc leave or slot delete, checked via `App::lateania_recently_active` (5 min) — and hopping in calls `enter_lateania()` to re-join the remembered slot, skipping character select. Then the roguelikes (NetHack/DCSS/Brogue) with a running (detached) game; inside one of those ` is intercepted by `App::handle_input` and re-enters this cycle as a detach (the running door state survives `set_screen`), so hopping in is a plain screen switch and the same key hops onward or out. |
